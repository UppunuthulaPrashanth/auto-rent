...
$serviceRes = $db->query("SELECT * FROM services WHERE serviceSlug = ?s", $slug);
$serviceRow = mysqli_fetch_assoc($serviceRes);

$serviceTypeID = $serviceRow['serviceTypeID'];
$serviceName   = $serviceRow['serviceName'];
$headerImage   = $serviceRow['headerImage'];
$description   = $serviceRow['description'];



$serviceTypeRes = $db->query("SELECT * FROM mtr_service_type WHERE serviceTypeID = ?s", $serviceTypeID);
$serviceTypeRow = mysqli_fetch_assoc($serviceTypeRes);

$serviceTypeName = $serviceTypeRow['serviceTypeName'];
...
<html lang="en">
   ...
   <body>
      ...
      <div class="theme-hero-area-bg-wrap">
        <div class="theme-hero-area-bg" style="background-image:url('uploads/services/<?php echo $headerImage?>');"></div>
          ...
          <div class="row">
            <div class="col-md-8 theme-page-header-abs">
              <div class="theme-page-header theme-page-header-lg">
                <h1 class="theme-page-header-title"><?php echo $serviceTypeName;?></h1>
                <p class="theme-page-header-subtitle"><?php echo $serviceName;?></p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="col-md-4 ">
                <div class="sticky-col _pv-45 _mob-h">
                  <div class="theme-search-area theme-search-area-vert">
                    ...
                    <h1 class="theme-search-area-title"><?php echo $row['pageTitle'];?></h1>
                    <p class="theme-search-area-subtitle"><?php echo $row['subTitle'];?></p>
                    </div>
