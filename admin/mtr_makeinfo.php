...
//
// Table class for mtr_make
//
class cmtr_make extends cTable {
	var $makeID;
	var $make;
	var $so;
	var $active;
	...
	function __construct() {
		...
		$this->TableVar = 'mtr_make';
		$this->TableName = 'mtr_make';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`mtr_make`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// makeID
		$this->makeID = new cField('mtr_make', 'mtr_make', 'x_makeID', 'makeID', '`makeID`', '`makeID`', 3, -1, FALSE, '`makeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// make
		$this->make = new cField('mtr_make', 'mtr_make', 'x_make', 'make', '`make`', '`make`', 200, -1, FALSE, '`make`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->make->Sortable = TRUE; // Allow sort
		$this->fields['make'] = &$this->make;

		// so
		$this->so = new cField('mtr_make', 'mtr_make', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('mtr_make', 'mtr_make', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`mtr_make`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->makeID->setDbValue($conn->Insert_ID());
			$rs['makeID'] = $this->makeID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('makeID', $rs))
				ew_AddFilter($where, ew_QuotedName('makeID', $this->DBID) . '=' . ew_QuotedValue($rs['makeID'], $this->makeID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->makeID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->makeID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@makeID@", ew_AdjustSql($this->makeID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "mtr_makeedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "mtr_makeadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("mtr_makeview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("mtr_makeview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "mtr_makeadd.php?" . $this->UrlParm($parm);
		else
			$url = "mtr_makeadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("mtr_makeedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("mtr_makeadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("mtr_makedelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "makeID:" . ew_VarToJson($this->makeID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->makeID->CurrentValue)) {
			$sUrl .= "makeID=" . urlencode($this->makeID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["makeID"]))
				$arKeys[] = $_POST["makeID"];
			elseif (isset($_GET["makeID"]))
				$arKeys[] = $_GET["makeID"];
			else
