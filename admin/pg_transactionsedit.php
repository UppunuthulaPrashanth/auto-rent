...
$pg_transactions_edit = NULL; // Initialize page object first

class cpg_transactions_edit extends cpg_transactions {
	...
	// Page object name
	var $PageObjName = 'pg_transactions_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_transactionID")) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// bookingModule
			$this->bookingModule->SetDbValueDef($rsnew, $this->bookingModule->CurrentValue, NULL, $this->bookingModule->ReadOnly);

			// order_id
			$this->order_id->SetDbValueDef($rsnew, $this->order_id->CurrentValue, NULL, $this->order_id->ReadOnly);

			// tracking_id
			$this->tracking_id->SetDbValueDef($rsnew, $this->tracking_id->CurrentValue, NULL, $this->tracking_id->ReadOnly);

			// bank_ref_no
			$this->bank_ref_no->SetDbValueDef($rsnew, $this->bank_ref_no->CurrentValue, NULL, $this->bank_ref_no->ReadOnly);

			// order_status
			$this->order_status->SetDbValueDef($rsnew, $this->order_status->CurrentValue, NULL, $this->order_status->ReadOnly);

			// failure_message
			$this->failure_message->SetDbValueDef($rsnew, $this->failure_message->CurrentValue, NULL, $this->failure_message->ReadOnly);

			// payment_mode
			$this->payment_mode->SetDbValueDef($rsnew, $this->payment_mode->CurrentValue, NULL, $this->payment_mode->ReadOnly);

			// card_name
			$this->card_name->SetDbValueDef($rsnew, $this->card_name->CurrentValue, NULL, $this->card_name->ReadOnly);

			// status_code
			$this->status_code->SetDbValueDef($rsnew, $this->status_code->CurrentValue, NULL, $this->status_code->ReadOnly);

			// status_message
			$this->status_message->SetDbValueDef($rsnew, $this->status_message->CurrentValue, NULL, $this->status_message->ReadOnly);
			...
			// billing_country
			$this->billing_country->SetDbValueDef($rsnew, $this->billing_country->CurrentValue, NULL, $this->billing_country->ReadOnly);

			// billing_tel
			$this->billing_tel->SetDbValueDef($rsnew, $this->billing_tel->CurrentValue, NULL, $this->billing_tel->ReadOnly);

			// billing_email
			$this->billing_email->SetDbValueDef($rsnew, $this->billing_email->CurrentValue, NULL, $this->billing_email->ReadOnly);

			// delivery_name
			$this->delivery_name->SetDbValueDef($rsnew, $this->delivery_name->CurrentValue, NULL, $this->delivery_name->ReadOnly);

			// delivery_address
			$this->delivery_address->SetDbValueDef($rsnew, $this->delivery_address->CurrentValue, NULL, $this->delivery_address->ReadOnly);

			// delivery_city
			$this->delivery_city->SetDbValueDef($rsnew, $this->delivery_city->CurrentValue, NULL, $this->delivery_city->ReadOnly);

			// delivery_state
			$this->delivery_state->SetDbValueDef($rsnew, $this->delivery_state->CurrentValue, NULL, $this->delivery_state->ReadOnly);

			// delivery_zip
			$this->delivery_zip->SetDbValueDef($rsnew, $this->delivery_zip->CurrentValue, NULL, $this->delivery_zip->ReadOnly);

			// delivery_country
			$this->delivery_country->SetDbValueDef($rsnew, $this->delivery_country->CurrentValue, NULL, $this->delivery_country->ReadOnly);

			// delivery_tel
			$this->delivery_tel->SetDbValueDef($rsnew, $this->delivery_tel->CurrentValue, NULL, $this->delivery_tel->ReadOnly);

			// merchant_param1
			$this->merchant_param1->SetDbValueDef($rsnew, $this->merchant_param1->CurrentValue, NULL, $this->merchant_param1->ReadOnly);

			// merchant_param2
			$this->merchant_param2->SetDbValueDef($rsnew, $this->merchant_param2->CurrentValue, NULL, $this->merchant_param2->ReadOnly);

			// merchant_param3
			$this->merchant_param3->SetDbValueDef($rsnew, $this->merchant_param3->CurrentValue, NULL, $this->merchant_param3->ReadOnly);

			// merchant_param4
			$this->merchant_param4->SetDbValueDef($rsnew, $this->merchant_param4->CurrentValue, NULL, $this->merchant_param4->ReadOnly);

			// merchant_param5
			$this->merchant_param5->SetDbValueDef($rsnew, $this->merchant_param5->CurrentValue, NULL, $this->merchant_param5->ReadOnly);

			// vault
			$this->vault->SetDbValueDef($rsnew, $this->vault->CurrentValue, NULL, $this->vault->ReadOnly);

			// offer_type
			$this->offer_type->SetDbValueDef($rsnew, $this->offer_type->CurrentValue, NULL, $this->offer_type->ReadOnly);

			// offer_code
			$this->offer_code->SetDbValueDef($rsnew, $this->offer_code->CurrentValue, NULL, $this->offer_code->ReadOnly);

			// discount_value
			$this->discount_value->SetDbValueDef($rsnew, $this->discount_value->CurrentValue, NULL, $this->discount_value->ReadOnly);

			// mer_amount
			$this->mer_amount->SetDbValueDef($rsnew, $this->mer_amount->CurrentValue, NULL, $this->mer_amount->ReadOnly);

			// eci_value
			$this->eci_value->SetDbValueDef($rsnew, $this->eci_value->CurrentValue, NULL, $this->eci_value->ReadOnly);

			// card_holder_name
			$this->card_holder_name->SetDbValueDef($rsnew, $this->card_holder_name->CurrentValue, NULL, $this->card_holder_name->ReadOnly);

			// bank_qsi_no
			$this->bank_qsi_no->SetDbValueDef($rsnew, $this->bank_qsi_no->CurrentValue, NULL, $this->bank_qsi_no->ReadOnly);

			// bank_receipt_no
			$this->bank_receipt_no->SetDbValueDef($rsnew, $this->bank_receipt_no->CurrentValue, NULL, $this->bank_receipt_no->ReadOnly);

			// merchant_param6
			$this->merchant_param6->SetDbValueDef($rsnew, $this->merchant_param6->CurrentValue, NULL, $this->merchant_param6->ReadOnly);
...
// Create page object
if (!isset($pg_transactions_edit)) $pg_transactions_edit = new cpg_transactions_edit();

// Page init
$pg_transactions_edit->Page_Init();

// Page main
$pg_transactions_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pg_transactions_edit->Page_Render();
?>
...
var CurrentForm = fpg_transactionsedit = new ew_Form("fpg_transactionsedit", "edit");

// Validate form
fpg_transactionsedit.Validate = function() {
...
fpg_transactionsedit.Form_CustomValidate = 
...
fpg_transactionsedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
fpg_transactionsedit.MultiPage = new ew_MultiPage("fpg_transactionsedit");

// Dynamic selection lists
fpg_transactionsedit.Lists["x__userID"] = {"LinkField":"x__userID","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstName","x_lastName","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fpg_transactionsedit.Lists["x__userID"].Data = "<?php echo $pg_transactions_edit->_userID->LookupFilterQuery(FALSE, "edit") ?>";
fpg_transactionsedit.Lists["x_bookingModule"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpg_transactionsedit.Lists["x_bookingModule"].Options = <?php echo json_encode($pg_transactions_edit->bookingModule->Options()) ?>;
...
<?php $pg_transactions_edit->ShowPageHeader(); ?>
<?php
$pg_transactions_edit->ShowMessage();
?>
<form name="fpg_transactionsedit" id="fpg_transactionsedit" class="<?php echo $pg_transactions_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pg_transactions_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pg_transactions_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="pg_transactions">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($pg_transactions_edit->IsModal) ?>">
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="pg_transactions_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $pg_transactions_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $pg_transactions_edit->MultiPages->TabStyle("1") ?>><a href="#tab_pg_transactions1" data-toggle="tab"><?php echo $pg_transactions->PageCaption(1) ?></a></li>
		<li<?php echo $pg_transactions_edit->MultiPages->TabStyle("2") ?>><a href="#tab_pg_transactions2" data-toggle="tab"><?php echo $pg_transactions->PageCaption(2) ?></a></li>
		<li<?php echo $pg_transactions_edit->MultiPages->TabStyle("3") ?>><a href="#tab_pg_transactions3" data-toggle="tab"><?php echo $pg_transactions->PageCaption(3) ?></a></li>
		<li<?php echo $pg_transactions_edit->MultiPages->TabStyle("4") ?>><a href="#tab_pg_transactions4" data-toggle="tab"><?php echo $pg_transactions->PageCaption(4) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $pg_transactions_edit->MultiPages->PageStyle("1") ?>" id="tab_pg_transactions1"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($pg_transactions->transactionID->Visible) { // transactionID ?>
	<div id="r_transactionID" class="form-group">
		<label id="elh_pg_transactions_transactionID" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->transactionID->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->transactionID->CellAttributes() ?>>
<span id="el_pg_transactions_transactionID">
<span<?php echo $pg_transactions->transactionID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $pg_transactions->transactionID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="pg_transactions" data-field="x_transactionID" data-page="1" name="x_transactionID" id="x_transactionID" value="<?php echo ew_HtmlEncode($pg_transactions->transactionID->CurrentValue) ?>">
<?php echo $pg_transactions->transactionID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pg_transactions->_userID->Visible) { // userID ?>
	<div id="r__userID" class="form-group">
		<label id="elh_pg_transactions__userID" for="x__userID" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->_userID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->_userID->CellAttributes() ?>>
...
<?php if ($pg_transactions->bookingNumber->Visible) { // bookingNumber ?>
	<div id="r_bookingNumber" class="form-group">
		<label id="elh_pg_transactions_bookingNumber" for="x_bookingNumber" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->bookingNumber->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->bookingNumber->CellAttributes() ?>>
...
<?php if ($pg_transactions->bookingModule->Visible) { // bookingModule ?>
	<div id="r_bookingModule" class="form-group">
		<label id="elh_pg_transactions_bookingModule" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->bookingModule->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->bookingModule->CellAttributes() ?>>
...
<?php if ($pg_transactions->order_id->Visible) { // order_id ?>
	<div id="r_order_id" class="form-group">
		<label id="elh_pg_transactions_order_id" for="x_order_id" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->order_id->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->order_id->CellAttributes() ?>>
...
<?php if ($pg_transactions->tracking_id->Visible) { // tracking_id ?>
	<div id="r_tracking_id" class="form-group">
		<label id="elh_pg_transactions_tracking_id" for="x_tracking_id" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->tracking_id->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->tracking_id->CellAttributes() ?>>
...
<?php if ($pg_transactions->bank_ref_no->Visible) { // bank_ref_no ?>
	<div id="r_bank_ref_no" class="form-group">
		<label id="elh_pg_transactions_bank_ref_no" for="x_bank_ref_no" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->bank_ref_no->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->bank_ref_no->CellAttributes() ?>>
...
<?php if ($pg_transactions->order_status->Visible) { // order_status ?>
	<div id="r_order_status" class="form-group">
		<label id="elh_pg_transactions_order_status" for="x_order_status" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->order_status->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->order_status->CellAttributes() ?>>
...
<?php if ($pg_transactions->failure_message->Visible) { // failure_message ?>
	<div id="r_failure_message" class="form-group">
		<label id="elh_pg_transactions_failure_message" for="x_failure_message" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->failure_message->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->failure_message->CellAttributes() ?>>
...
<?php if ($pg_transactions->payment_mode->Visible) { // payment_mode ?>
	<div id="r_payment_mode" class="form-group">
		<label id="elh_pg_transactions_payment_mode" for="x_payment_mode" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->payment_mode->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->payment_mode->CellAttributes() ?>>
...
<?php if ($pg_transactions->card_name->Visible) { // card_name ?>
	<div id="r_card_name" class="form-group">
		<label id="elh_pg_transactions_card_name" for="x_card_name" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->card_name->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->card_name->CellAttributes() ?>>
...
<?php if ($pg_transactions->status_code->Visible) { // status_code ?>
	<div id="r_status_code" class="form-group">
		<label id="elh_pg_transactions_status_code" for="x_status_code" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->status_code->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->status_code->CellAttributes() ?>>
...
<?php if ($pg_transactions->status_message->Visible) { // status_message ?>
	<div id="r_status_message" class="form-group">
		<label id="elh_pg_transactions_status_message" for="x_status_message" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->status_message->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->status_message->CellAttributes() ?>>
...
<?php if ($pg_transactions->currency->Visible) { // currency ?>
	<div id="r_currency" class="form-group">
		<label id="elh_pg_transactions_currency" for="x_currency" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->currency->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->currency->CellAttributes() ?>>
...
<?php if ($pg_transactions->amount->Visible) { // amount ?>
	<div id="r_amount" class="form-group">
		<label id="elh_pg_transactions_amount" for="x_amount" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->amount->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->amount->CellAttributes() ?>>
...
<?php if ($pg_transactions->vault->Visible) { // vault ?>
	<div id="r_vault" class="form-group">
		<label id="elh_pg_transactions_vault" for="x_vault" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->vault->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->vault->CellAttributes() ?>>
...
<?php if ($pg_transactions->offer_type->Visible) { // offer_type ?>
	<div id="r_offer_type" class="form-group">
		<label id="elh_pg_transactions_offer_type" for="x_offer_type" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->offer_type->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->offer_type->CellAttributes() ?>>
...
<?php if ($pg_transactions->offer_code->Visible) { // offer_code ?>
	<div id="r_offer_code" class="form-group">
		<label id="elh_pg_transactions_offer_code" for="x_offer_code" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->offer_code->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->offer_code->CellAttributes() ?>>
...
<?php if ($pg_transactions->discount_value->Visible) { // discount_value ?>
	<div id="r_discount_value" class="form-group">
		<label id="elh_pg_transactions_discount_value" for="x_discount_value" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->discount_value->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->discount_value->CellAttributes() ?>>
...
<?php if ($pg_transactions->mer_amount->Visible) { // mer_amount ?>
	<div id="r_mer_amount" class="form-group">
		<label id="elh_pg_transactions_mer_amount" for="x_mer_amount" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->mer_amount->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->mer_amount->CellAttributes() ?>>
...
<?php if ($pg_transactions->eci_value->Visible) { // eci_value ?>
	<div id="r_eci_value" class="form-group">
		<label id="elh_pg_transactions_eci_value" for="x_eci_value" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->eci_value->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->eci_value->CellAttributes() ?>>
...
<?php if ($pg_transactions->card_holder_name->Visible) { // card_holder_name ?>
	<div id="r_card_holder_name" class="form-group">
		<label id="elh_pg_transactions_card_holder_name" for="x_card_holder_name" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->card_holder_name->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->card_holder_name->CellAttributes() ?>>
...
<?php if ($pg_transactions->bank_qsi_no->Visible) { // bank_qsi_no ?>
	<div id="r_bank_qsi_no" class="form-group">
		<label id="elh_pg_transactions_bank_qsi_no" for="x_bank_qsi_no" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->bank_qsi_no->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->bank_qsi_no->CellAttributes() ?>>
...
<?php if ($pg_transactions->bank_receipt_no->Visible) { // bank_receipt_no ?>
	<div id="r_bank_receipt_no" class="form-group">
		<label id="elh_pg_transactions_bank_receipt_no" for="x_bank_receipt_no" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->bank_receipt_no->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->bank_receipt_no->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $pg_transactions_edit->MultiPages->PageStyle("2") ?>" id="tab_pg_transactions2"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($pg_transactions->billing_name->Visible) { // billing_name ?>
	<div id="r_billing_name" class="form-group">
		<label id="elh_pg_transactions_billing_name" for="x_billing_name" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_name->CellAttributes() ?>>
...
<?php if ($pg_transactions->billing_address->Visible) { // billing_address ?>
	<div id="r_billing_address" class="form-group">
		<label id="elh_pg_transactions_billing_address" for="x_billing_address" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_address->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_address->CellAttributes() ?>>
...
<?php if ($pg_transactions->billing_city->Visible) { // billing_city ?>
	<div id="r_billing_city" class="form-group">
		<label id="elh_pg_transactions_billing_city" for="x_billing_city" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_city->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_city->CellAttributes() ?>>
...
<?php if ($pg_transactions->billing_state->Visible) { // billing_state ?>
	<div id="r_billing_state" class="form-group">
		<label id="elh_pg_transactions_billing_state" for="x_billing_state" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_state->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_state->CellAttributes() ?>>
...
<?php if ($pg_transactions->billing_zip->Visible) { // billing_zip ?>
	<div id="r_billing_zip" class="form-group">
		<label id="elh_pg_transactions_billing_zip" for="x_billing_zip" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_zip->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_zip->CellAttributes() ?>>
...
<?php if ($pg_transactions->billing_country->Visible) { // billing_country ?>
	<div id="r_billing_country" class="form-group">
		<label id="elh_pg_transactions_billing_country" for="x_billing_country" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_country->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_country->CellAttributes() ?>>
...
<?php if ($pg_transactions->billing_tel->Visible) { // billing_tel ?>
	<div id="r_billing_tel" class="form-group">
		<label id="elh_pg_transactions_billing_tel" for="x_billing_tel" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_tel->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_tel->CellAttributes() ?>>
...
<?php if ($pg_transactions->billing_email->Visible) { // billing_email ?>
	<div id="r_billing_email" class="form-group">
		<label id="elh_pg_transactions_billing_email" for="x_billing_email" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->billing_email->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->billing_email->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $pg_transactions_edit->MultiPages->PageStyle("3") ?>" id="tab_pg_transactions3"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($pg_transactions->delivery_name->Visible) { // delivery_name ?>
	<div id="r_delivery_name" class="form-group">
		<label id="elh_pg_transactions_delivery_name" for="x_delivery_name" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->delivery_name->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->delivery_name->CellAttributes() ?>>
...
<?php if ($pg_transactions->delivery_address->Visible) { // delivery_address ?>
	<div id="r_delivery_address" class="form-group">
		<label id="elh_pg_transactions_delivery_address" for="x_delivery_address" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->delivery_address->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->delivery_address->CellAttributes() ?>>
...
<?php if ($pg_transactions->delivery_city->Visible) { // delivery_city ?>
	<div id="r_delivery_city" class="form-group">
		<label id="elh_pg_transactions_delivery_city" for="x_delivery_city" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->delivery_city->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->delivery_city->CellAttributes() ?>>
...
<?php if ($pg_transactions->delivery_state->Visible) { // delivery_state ?>
	<div id="r_delivery_state" class="form-group">
		<label id="elh_pg_transactions_delivery_state" for="x_delivery_state" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->delivery_state->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->delivery_state->CellAttributes() ?>>
...
<?php if ($pg_transactions->delivery_zip->Visible) { // delivery_zip ?>
	<div id="r_delivery_zip" class="form-group">
		<label id="elh_pg_transactions_delivery_zip" for="x_delivery_zip" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->delivery_zip->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->delivery_zip->CellAttributes() ?>>
...
<?php if ($pg_transactions->delivery_country->Visible) { // delivery_country ?>
	<div id="r_delivery_country" class="form-group">
		<label id="elh_pg_transactions_delivery_country" for="x_delivery_country" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->delivery_country->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->delivery_country->CellAttributes() ?>>
...
<?php if ($pg_transactions->delivery_tel->Visible) { // delivery_tel ?>
	<div id="r_delivery_tel" class="form-group">
		<label id="elh_pg_transactions_delivery_tel" for="x_delivery_tel" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->delivery_tel->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->delivery_tel->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $pg_transactions_edit->MultiPages->PageStyle("4") ?>" id="tab_pg_transactions4"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($pg_transactions->merchant_param1->Visible) { // merchant_param1 ?>
	<div id="r_merchant_param1" class="form-group">
		<label id="elh_pg_transactions_merchant_param1" for="x_merchant_param1" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->merchant_param1->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->merchant_param1->CellAttributes() ?>>
...
<?php if ($pg_transactions->merchant_param2->Visible) { // merchant_param2 ?>
	<div id="r_merchant_param2" class="form-group">
		<label id="elh_pg_transactions_merchant_param2" for="x_merchant_param2" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->merchant_param2->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->merchant_param2->CellAttributes() ?>>
...
<?php if ($pg_transactions->merchant_param3->Visible) { // merchant_param3 ?>
	<div id="r_merchant_param3" class="form-group">
		<label id="elh_pg_transactions_merchant_param3" for="x_merchant_param3" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->merchant_param3->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->merchant_param3->CellAttributes() ?>>
...
<?php if ($pg_transactions->merchant_param4->Visible) { // merchant_param4 ?>
	<div id="r_merchant_param4" class="form-group">
		<label id="elh_pg_transactions_merchant_param4" for="x_merchant_param4" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->merchant_param4->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->merchant_param4->CellAttributes() ?>>
...
<?php if ($pg_transactions->merchant_param5->Visible) { // merchant_param5 ?>
	<div id="r_merchant_param5" class="form-group">
		<label id="elh_pg_transactions_merchant_param5" for="x_merchant_param5" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->merchant_param5->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->merchant_param5->CellAttributes() ?>>
...
<?php if ($pg_transactions->merchant_param6->Visible) { // merchant_param6 ?>
	<div id="r_merchant_param6" class="form-group">
		<label id="elh_pg_transactions_merchant_param6" for="x_merchant_param6" class="<?php echo $pg_transactions_edit->LeftColumnClass ?>"><?php echo $pg_transactions->merchant_param6->FldCaption() ?></label>
		<div class="<?php echo $pg_transactions_edit->RightColumnClass ?>"><div<?php echo $pg_transactions->merchant_param6->CellAttributes() ?>>
...
<?php if (!$pg_transactions_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $pg_transactions_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pg_transactions_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fpg_transactionsedit.Init();
</script>
<?php
$pg_transactions_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$pg_transactions_edit->Page_Terminate();
?>
