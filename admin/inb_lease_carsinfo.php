...
//
// Table class for inb_lease_cars
//
class cinb_lease_cars extends cTable {
	var $id;
	var $leaseCarType;
	var $fullname;
	var $_email;
	...
	function __construct() {
		...
		$this->TableVar = 'inb_lease_cars';
		$this->TableName = 'inb_lease_cars';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`inb_lease_cars`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('inb_lease_cars', 'inb_lease_cars', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// leaseCarType
		$this->leaseCarType = new cField('inb_lease_cars', 'inb_lease_cars', 'x_leaseCarType', 'leaseCarType', '`leaseCarType`', '`leaseCarType`', 200, -1, FALSE, '`leaseCarType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->leaseCarType->Sortable = TRUE; // Allow sort
		$this->fields['leaseCarType'] = &$this->leaseCarType;

		// fullname
		$this->fullname = new cField('inb_lease_cars', 'inb_lease_cars', 'x_fullname', 'fullname', '`fullname`', '`fullname`', 200, -1, FALSE, '`fullname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fullname->Sortable = TRUE; // Allow sort
		$this->fields['fullname'] = &$this->fullname;

		// email
		$this->_email = new cField('inb_lease_cars', 'inb_lease_cars', 'x__email', 'email', '`email`', '`email`', 200, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// phone
		$this->phone = new cField('inb_lease_cars', 'inb_lease_cars', 'x_phone', 'phone', '`phone`', '`phone`', 200, -1, FALSE, '`phone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->phone->Sortable = TRUE; // Allow sort
		$this->fields['phone'] = &$this->phone;

		// companyName
		$this->companyName = new cField('inb_lease_cars', 'inb_lease_cars', 'x_companyName', 'companyName', '`companyName`', '`companyName`', 200, -1, FALSE, '`companyName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->companyName->Sortable = TRUE; // Allow sort
		$this->fields['companyName'] = &$this->companyName;

		// specificRequirement
		$this->specificRequirement = new cField('inb_lease_cars', 'inb_lease_cars', 'x_specificRequirement', 'specificRequirement', '`specificRequirement`', '`specificRequirement`', 201, -1, FALSE, '`specificRequirement`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->specificRequirement->Sortable = TRUE; // Allow sort
		$this->fields['specificRequirement'] = &$this->specificRequirement;

		// dateCreated
		$this->dateCreated = new cField('inb_lease_cars', 'inb_lease_cars', 'x_dateCreated', 'dateCreated', '`dateCreated`', ew_CastDateFieldForLike('`dateCreated`', 7, "DB"), 135, 7, FALSE, '`dateCreated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`inb_lease_cars`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "inb_lease_carsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "inb_lease_carsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("inb_lease_carsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("inb_lease_carsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "inb_lease_carsadd.php?" . $this->UrlParm($parm);
		else
			$url = "inb_lease_carsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("inb_lease_carsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("inb_lease_carsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("inb_lease_carsdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->leaseCarType->setDbValue($rs->fields('leaseCarType'));
	...
	function RenderEditRow() {
		...
		$this->leaseCarType->EditValue = $this->leaseCarType->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->leaseCarType->Exportable) $Doc->ExportCaption($this->leaseCarType);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->leaseCarType->Exportable) $Doc->ExportField($this->leaseCarType);
