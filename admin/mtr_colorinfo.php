...
//
// Table class for mtr_color
//
class cmtr_color extends cTable {
	var $colorID;
	var $color;
	var $so;
	var $active;
	...
	function __construct() {
		...
		$this->TableVar = 'mtr_color';
		$this->TableName = 'mtr_color';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`mtr_color`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// colorID
		$this->colorID = new cField('mtr_color', 'mtr_color', 'x_colorID', 'colorID', '`colorID`', '`colorID`', 3, -1, FALSE, '`colorID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// color
		$this->color = new cField('mtr_color', 'mtr_color', 'x_color', 'color', '`color`', '`color`', 200, -1, FALSE, '`color`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->color->Sortable = TRUE; // Allow sort
		$this->fields['color'] = &$this->color;

		// so
		$this->so = new cField('mtr_color', 'mtr_color', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('mtr_color', 'mtr_color', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`mtr_color`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->colorID->setDbValue($conn->Insert_ID());
			$rs['colorID'] = $this->colorID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('colorID', $rs))
				ew_AddFilter($where, ew_QuotedName('colorID', $this->DBID) . '=' . ew_QuotedValue($rs['colorID'], $this->colorID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->colorID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->colorID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@colorID@", ew_AdjustSql($this->colorID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "mtr_coloredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "mtr_coloradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("mtr_colorview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("mtr_colorview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "mtr_coloradd.php?" . $this->UrlParm($parm);
		else
			$url = "mtr_coloradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("mtr_coloredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("mtr_coloradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("mtr_colordelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "colorID:" . ew_VarToJson($this->colorID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->colorID->CurrentValue)) {
			$sUrl .= "colorID=" . urlencode($this->colorID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["colorID"]))
				$arKeys[] = $_POST["colorID"];
			elseif (isset($_GET["colorID"]))
				$arKeys[] = $_GET["colorID"];
			else
