...
$used_cars_edit = NULL; // Initialize page object first

class cused_cars_edit extends cused_cars {
	...
	// Page object name
	var $PageObjName = 'used_cars_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_userCarID")) {
				$this->userCarID->setFormValue($objForm->GetValue("x_userCarID"));
			}
		} else {
	...
	function LoadFormValues() {
		...
		if (!$this->userCarID->FldIsDetailKey)
			$this->userCarID->setFormValue($objForm->GetValue("x_userCarID"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->userCarID->CurrentValue = $this->userCarID->FormValue;
	...
	function RenderRow() {
		...
		$this->active->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_01);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_02);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_03);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_04);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_05);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_06);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_07);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_08);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_09);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_10);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_11);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_12);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// modelID
			$this->modelID->SetDbValueDef($rsnew, $this->modelID->CurrentValue, NULL, $this->modelID->ReadOnly);

			// slug
			$this->slug->SetDbValueDef($rsnew, $this->slug->CurrentValue, "", $this->slug->ReadOnly);

			// yearID
			$this->yearID->SetDbValueDef($rsnew, $this->yearID->CurrentValue, NULL, $this->yearID->ReadOnly);

			// kilometers
			$this->kilometers->SetDbValueDef($rsnew, $this->kilometers->CurrentValue, NULL, $this->kilometers->ReadOnly);

			// priceAED
			$this->priceAED->SetDbValueDef($rsnew, $this->priceAED->CurrentValue, NULL, $this->priceAED->ReadOnly);

			// priceUSD
			$this->priceUSD->SetDbValueDef($rsnew, $this->priceUSD->CurrentValue, NULL, $this->priceUSD->ReadOnly);

			// priceOMR
			$this->priceOMR->SetDbValueDef($rsnew, $this->priceOMR->CurrentValue, NULL, $this->priceOMR->ReadOnly);

			// priceSAR
			$this->priceSAR->SetDbValueDef($rsnew, $this->priceSAR->CurrentValue, NULL, $this->priceSAR->ReadOnly);

			// description
			$this->description->SetDbValueDef($rsnew, $this->description->CurrentValue, NULL, $this->description->ReadOnly);

			// fuelTypeID
			$this->fuelTypeID->SetDbValueDef($rsnew, $this->fuelTypeID->CurrentValue, NULL, $this->fuelTypeID->ReadOnly);

			// regionalID
			$this->regionalID->SetDbValueDef($rsnew, $this->regionalID->CurrentValue, NULL, $this->regionalID->ReadOnly);

			// warrantyID
			$this->warrantyID->SetDbValueDef($rsnew, $this->warrantyID->CurrentValue, NULL, $this->warrantyID->ReadOnly);

			// noOfDoors
			$this->noOfDoors->SetDbValueDef($rsnew, $this->noOfDoors->CurrentValue, NULL, $this->noOfDoors->ReadOnly);

			// transmissionTypeID
			$this->transmissionTypeID->SetDbValueDef($rsnew, $this->transmissionTypeID->CurrentValue, NULL, $this->transmissionTypeID->ReadOnly);

			// cylinderID
			$this->cylinderID->SetDbValueDef($rsnew, $this->cylinderID->CurrentValue, NULL, $this->cylinderID->ReadOnly);

			// engine
			$this->engine->SetDbValueDef($rsnew, $this->engine->CurrentValue, NULL, $this->engine->ReadOnly);

			// colorID
			$this->colorID->SetDbValueDef($rsnew, $this->colorID->CurrentValue, NULL, $this->colorID->ReadOnly);

			// bodyConditionID
			$this->bodyConditionID->SetDbValueDef($rsnew, $this->bodyConditionID->CurrentValue, NULL, $this->bodyConditionID->ReadOnly);

			// summary
			$this->summary->SetDbValueDef($rsnew, $this->summary->CurrentValue, NULL, $this->summary->ReadOnly);

			// term
			$this->term->SetDbValueDef($rsnew, $this->term->CurrentValue, NULL, $this->term->ReadOnly);
			...
			// img_01
			if ($this->img_01->Visible && !$this->img_01->ReadOnly && !$this->img_01->Upload->KeepFile) {
				$this->img_01->Upload->DbValue = $rsold['img_01']; // Get original value
			...
			// img_02
			if ($this->img_02->Visible && !$this->img_02->ReadOnly && !$this->img_02->Upload->KeepFile) {
				$this->img_02->Upload->DbValue = $rsold['img_02']; // Get original value
			...
			// img_03
			if ($this->img_03->Visible && !$this->img_03->ReadOnly && !$this->img_03->Upload->KeepFile) {
				$this->img_03->Upload->DbValue = $rsold['img_03']; // Get original value
			...
			// img_04
			if ($this->img_04->Visible && !$this->img_04->ReadOnly && !$this->img_04->Upload->KeepFile) {
				$this->img_04->Upload->DbValue = $rsold['img_04']; // Get original value
			...
			// img_05
			if ($this->img_05->Visible && !$this->img_05->ReadOnly && !$this->img_05->Upload->KeepFile) {
				$this->img_05->Upload->DbValue = $rsold['img_05']; // Get original value
			...
			// img_06
			if ($this->img_06->Visible && !$this->img_06->ReadOnly && !$this->img_06->Upload->KeepFile) {
				$this->img_06->Upload->DbValue = $rsold['img_06']; // Get original value
			...
			// img_07
			if ($this->img_07->Visible && !$this->img_07->ReadOnly && !$this->img_07->Upload->KeepFile) {
				$this->img_07->Upload->DbValue = $rsold['img_07']; // Get original value
			...
			// img_08
			if ($this->img_08->Visible && !$this->img_08->ReadOnly && !$this->img_08->Upload->KeepFile) {
				$this->img_08->Upload->DbValue = $rsold['img_08']; // Get original value
			...
			// img_09
			if ($this->img_09->Visible && !$this->img_09->ReadOnly && !$this->img_09->Upload->KeepFile) {
				$this->img_09->Upload->DbValue = $rsold['img_09']; // Get original value
			...
			// img_10
			if ($this->img_10->Visible && !$this->img_10->ReadOnly && !$this->img_10->Upload->KeepFile) {
				$this->img_10->Upload->DbValue = $rsold['img_10']; // Get original value
			...
			// img_11
			if ($this->img_11->Visible && !$this->img_11->ReadOnly && !$this->img_11->Upload->KeepFile) {
				$this->img_11->Upload->DbValue = $rsold['img_11']; // Get original value
			...
			// img_12
			if ($this->img_12->Visible && !$this->img_12->ReadOnly && !$this->img_12->Upload->KeepFile) {
				$this->img_12->Upload->DbValue = $rsold['img_12']; // Get original value
			...
			// extra_features
			$this->extra_features->SetDbValueDef($rsnew, $this->extra_features->CurrentValue, NULL, $this->extra_features->ReadOnly);
			...
			if ($this->img_01->Visible && !$this->img_01->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_01->Upload->FileName)) {
					...
					$this->img_01->SetDbValueDef($rsnew, $this->img_01->Upload->FileName, NULL, $this->img_01->ReadOnly);
				}
			}
			if ($this->img_02->Visible && !$this->img_02->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_02->Upload->FileName)) {
					...
					$this->img_02->SetDbValueDef($rsnew, $this->img_02->Upload->FileName, NULL, $this->img_02->ReadOnly);
				}
			}
			if ($this->img_03->Visible && !$this->img_03->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_03->Upload->FileName)) {
					...
					$this->img_03->SetDbValueDef($rsnew, $this->img_03->Upload->FileName, NULL, $this->img_03->ReadOnly);
				}
			}
			if ($this->img_04->Visible && !$this->img_04->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_04->Upload->FileName)) {
					...
					$this->img_04->SetDbValueDef($rsnew, $this->img_04->Upload->FileName, NULL, $this->img_04->ReadOnly);
				}
			}
			if ($this->img_05->Visible && !$this->img_05->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_05->Upload->FileName)) {
					...
					$this->img_05->SetDbValueDef($rsnew, $this->img_05->Upload->FileName, NULL, $this->img_05->ReadOnly);
				}
			}
			if ($this->img_06->Visible && !$this->img_06->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_06->Upload->FileName)) {
					...
					$this->img_06->SetDbValueDef($rsnew, $this->img_06->Upload->FileName, NULL, $this->img_06->ReadOnly);
				}
			}
			if ($this->img_07->Visible && !$this->img_07->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_07->Upload->FileName)) {
					...
					$this->img_07->SetDbValueDef($rsnew, $this->img_07->Upload->FileName, NULL, $this->img_07->ReadOnly);
				}
			}
			if ($this->img_08->Visible && !$this->img_08->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_08->Upload->FileName)) {
					...
					$this->img_08->SetDbValueDef($rsnew, $this->img_08->Upload->FileName, NULL, $this->img_08->ReadOnly);
				}
			}
			if ($this->img_09->Visible && !$this->img_09->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_09->Upload->FileName)) {
					...
					$this->img_09->SetDbValueDef($rsnew, $this->img_09->Upload->FileName, NULL, $this->img_09->ReadOnly);
				}
			}
			if ($this->img_10->Visible && !$this->img_10->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_10->Upload->FileName)) {
					...
					$this->img_10->SetDbValueDef($rsnew, $this->img_10->Upload->FileName, NULL, $this->img_10->ReadOnly);
				}
			}
			if ($this->img_11->Visible && !$this->img_11->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_11->Upload->FileName)) {
					...
					$this->img_11->SetDbValueDef($rsnew, $this->img_11->Upload->FileName, NULL, $this->img_11->ReadOnly);
				}
			}
			if ($this->img_12->Visible && !$this->img_12->Upload->KeepFile) {
				...
				if (!ew_Empty($this->img_12->Upload->FileName)) {
					...
					$this->img_12->SetDbValueDef($rsnew, $this->img_12->Upload->FileName, NULL, $this->img_12->ReadOnly);
				}
			}
...
// Create page object
if (!isset($used_cars_edit)) $used_cars_edit = new cused_cars_edit();

// Page init
$used_cars_edit->Page_Init();

// Page main
$used_cars_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$used_cars_edit->Page_Render();
?>
...
var CurrentForm = fused_carsedit = new ew_Form("fused_carsedit", "edit");

// Validate form
fused_carsedit.Validate = function() {
...
fused_carsedit.Form_CustomValidate = 
...
fused_carsedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
fused_carsedit.MultiPage = new ew_MultiPage("fused_carsedit");

// Dynamic selection lists
fused_carsedit.Lists["x_makeID"] = {"LinkField":"x_makeID","Ajax":true,"AutoFill":false,"DisplayFields":["x_make","","",""],"ParentFields":[],"ChildFields":["x_modelID"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_make"};
fused_carsedit.Lists["x_makeID"].Data = "<?php echo $used_cars_edit->makeID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_modelID"] = {"LinkField":"x_modelID","Ajax":true,"AutoFill":false,"DisplayFields":["x_model","","",""],"ParentFields":["x_makeID"],"ChildFields":[],"FilterFields":["x_makeID"],"Options":[],"Template":"","LinkTable":"mtr_model"};
fused_carsedit.Lists["x_modelID"].Data = "<?php echo $used_cars_edit->modelID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_yearID"] = {"LinkField":"x_yearID","Ajax":true,"AutoFill":false,"DisplayFields":["x_year","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_year"};
fused_carsedit.Lists["x_yearID"].Data = "<?php echo $used_cars_edit->yearID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_fuelTypeID"] = {"LinkField":"x_fuelTypeID","Ajax":true,"AutoFill":false,"DisplayFields":["x_fuelType","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_fuel_type"};
fused_carsedit.Lists["x_fuelTypeID"].Data = "<?php echo $used_cars_edit->fuelTypeID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_regionalID"] = {"LinkField":"x_regionalID","Ajax":true,"AutoFill":false,"DisplayFields":["x_regionalSpecs","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_regional_spec"};
fused_carsedit.Lists["x_regionalID"].Data = "<?php echo $used_cars_edit->regionalID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_warrantyID"] = {"LinkField":"x_warrantyID","Ajax":true,"AutoFill":false,"DisplayFields":["x_warrantyName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_warranty"};
fused_carsedit.Lists["x_warrantyID"].Data = "<?php echo $used_cars_edit->warrantyID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_transmissionTypeID"] = {"LinkField":"x_transmissionID","Ajax":true,"AutoFill":false,"DisplayFields":["x_transmission","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_transmission"};
fused_carsedit.Lists["x_transmissionTypeID"].Data = "<?php echo $used_cars_edit->transmissionTypeID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_cylinderID"] = {"LinkField":"x_cylinderID","Ajax":true,"AutoFill":false,"DisplayFields":["x_cylinder","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_cylinder"};
fused_carsedit.Lists["x_cylinderID"].Data = "<?php echo $used_cars_edit->cylinderID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_colorID"] = {"LinkField":"x_colorID","Ajax":true,"AutoFill":false,"DisplayFields":["x_color","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_color"};
fused_carsedit.Lists["x_colorID"].Data = "<?php echo $used_cars_edit->colorID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_bodyConditionID"] = {"LinkField":"x_bodyConditionID","Ajax":true,"AutoFill":false,"DisplayFields":["x_bodyCondition","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_body_condition"};
fused_carsedit.Lists["x_bodyConditionID"].Data = "<?php echo $used_cars_edit->bodyConditionID->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_term"] = {"LinkField":"x_termID","Ajax":true,"AutoFill":false,"DisplayFields":["x_term","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_term"};
fused_carsedit.Lists["x_term"].Data = "<?php echo $used_cars_edit->term->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_extra_features[]"] = {"LinkField":"x_featureID","Ajax":true,"AutoFill":false,"DisplayFields":["x_extraFeatures","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_extra_features"};
fused_carsedit.Lists["x_extra_features[]"].Data = "<?php echo $used_cars_edit->extra_features->LookupFilterQuery(FALSE, "edit") ?>";
fused_carsedit.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fused_carsedit.Lists["x_active"].Options = <?php echo json_encode($used_cars_edit->active->Options()) ?>;
...
<?php $used_cars_edit->ShowPageHeader(); ?>
<?php
$used_cars_edit->ShowMessage();
?>
<form name="fused_carsedit" id="fused_carsedit" class="<?php echo $used_cars_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($used_cars_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $used_cars_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="used_cars">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($used_cars_edit->IsModal) ?>">
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="used_cars_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $used_cars_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $used_cars_edit->MultiPages->TabStyle("1") ?>><a href="#tab_used_cars1" data-toggle="tab"><?php echo $used_cars->PageCaption(1) ?></a></li>
		<li<?php echo $used_cars_edit->MultiPages->TabStyle("2") ?>><a href="#tab_used_cars2" data-toggle="tab"><?php echo $used_cars->PageCaption(2) ?></a></li>
		<li<?php echo $used_cars_edit->MultiPages->TabStyle("3") ?>><a href="#tab_used_cars3" data-toggle="tab"><?php echo $used_cars->PageCaption(3) ?></a></li>
		<li<?php echo $used_cars_edit->MultiPages->TabStyle("4") ?>><a href="#tab_used_cars4" data-toggle="tab"><?php echo $used_cars->PageCaption(4) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $used_cars_edit->MultiPages->PageStyle("1") ?>" id="tab_used_cars1"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($used_cars->userCarID->Visible) { // userCarID ?>
	<div id="r_userCarID" class="form-group">
		<label id="elh_used_cars_userCarID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->userCarID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->userCarID->CellAttributes() ?>>
<span id="el_used_cars_userCarID">
<span<?php echo $used_cars->userCarID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $used_cars->userCarID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="used_cars" data-field="x_userCarID" data-page="1" name="x_userCarID" id="x_userCarID" value="<?php echo ew_HtmlEncode($used_cars->userCarID->CurrentValue) ?>">
<?php echo $used_cars->userCarID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($used_cars->makeID->Visible) { // makeID ?>
	<div id="r_makeID" class="form-group">
		<label id="elh_used_cars_makeID" for="x_makeID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->makeID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->makeID->CellAttributes() ?>>
...
<?php if ($used_cars->modelID->Visible) { // modelID ?>
	<div id="r_modelID" class="form-group">
		<label id="elh_used_cars_modelID" for="x_modelID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->modelID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->modelID->CellAttributes() ?>>
...
<?php if ($used_cars->slug->Visible) { // slug ?>
	<div id="r_slug" class="form-group">
		<label id="elh_used_cars_slug" for="x_slug" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->slug->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->slug->CellAttributes() ?>>
...
<?php if ($used_cars->yearID->Visible) { // yearID ?>
	<div id="r_yearID" class="form-group">
		<label id="elh_used_cars_yearID" for="x_yearID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->yearID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->yearID->CellAttributes() ?>>
...
<?php if ($used_cars->kilometers->Visible) { // kilometers ?>
	<div id="r_kilometers" class="form-group">
		<label id="elh_used_cars_kilometers" for="x_kilometers" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->kilometers->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->kilometers->CellAttributes() ?>>
...
<?php if ($used_cars->priceAED->Visible) { // priceAED ?>
	<div id="r_priceAED" class="form-group">
		<label id="elh_used_cars_priceAED" for="x_priceAED" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->priceAED->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->priceAED->CellAttributes() ?>>
...
<?php if ($used_cars->priceUSD->Visible) { // priceUSD ?>
	<div id="r_priceUSD" class="form-group">
		<label id="elh_used_cars_priceUSD" for="x_priceUSD" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->priceUSD->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->priceUSD->CellAttributes() ?>>
...
<?php if ($used_cars->priceOMR->Visible) { // priceOMR ?>
	<div id="r_priceOMR" class="form-group">
		<label id="elh_used_cars_priceOMR" for="x_priceOMR" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->priceOMR->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->priceOMR->CellAttributes() ?>>
...
<?php if ($used_cars->priceSAR->Visible) { // priceSAR ?>
	<div id="r_priceSAR" class="form-group">
		<label id="elh_used_cars_priceSAR" for="x_priceSAR" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->priceSAR->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->priceSAR->CellAttributes() ?>>
...
<?php if ($used_cars->so->Visible) { // so ?>
	<div id="r_so" class="form-group">
		<label id="elh_used_cars_so" for="x_so" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->so->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->so->CellAttributes() ?>>
...
<?php if ($used_cars->active->Visible) { // active ?>
	<div id="r_active" class="form-group">
		<label id="elh_used_cars_active" for="x_active" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->active->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->active->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $used_cars_edit->MultiPages->PageStyle("2") ?>" id="tab_used_cars2"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($used_cars->description->Visible) { // description ?>
	<div id="r_description" class="form-group">
		<label id="elh_used_cars_description" for="x_description" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->description->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->description->CellAttributes() ?>>
...
<?php if ($used_cars->fuelTypeID->Visible) { // fuelTypeID ?>
	<div id="r_fuelTypeID" class="form-group">
		<label id="elh_used_cars_fuelTypeID" for="x_fuelTypeID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->fuelTypeID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->fuelTypeID->CellAttributes() ?>>
...
<?php if ($used_cars->regionalID->Visible) { // regionalID ?>
	<div id="r_regionalID" class="form-group">
		<label id="elh_used_cars_regionalID" for="x_regionalID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->regionalID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->regionalID->CellAttributes() ?>>
...
<?php if ($used_cars->warrantyID->Visible) { // warrantyID ?>
	<div id="r_warrantyID" class="form-group">
		<label id="elh_used_cars_warrantyID" for="x_warrantyID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->warrantyID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->warrantyID->CellAttributes() ?>>
...
<?php if ($used_cars->noOfDoors->Visible) { // noOfDoors ?>
	<div id="r_noOfDoors" class="form-group">
		<label id="elh_used_cars_noOfDoors" for="x_noOfDoors" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->noOfDoors->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->noOfDoors->CellAttributes() ?>>
...
<?php if ($used_cars->transmissionTypeID->Visible) { // transmissionTypeID ?>
	<div id="r_transmissionTypeID" class="form-group">
		<label id="elh_used_cars_transmissionTypeID" for="x_transmissionTypeID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->transmissionTypeID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->transmissionTypeID->CellAttributes() ?>>
...
<?php if ($used_cars->cylinderID->Visible) { // cylinderID ?>
	<div id="r_cylinderID" class="form-group">
		<label id="elh_used_cars_cylinderID" for="x_cylinderID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->cylinderID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->cylinderID->CellAttributes() ?>>
...
<?php if ($used_cars->engine->Visible) { // engine ?>
	<div id="r_engine" class="form-group">
		<label id="elh_used_cars_engine" for="x_engine" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->engine->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->engine->CellAttributes() ?>>
...
<?php if ($used_cars->colorID->Visible) { // colorID ?>
	<div id="r_colorID" class="form-group">
		<label id="elh_used_cars_colorID" for="x_colorID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->colorID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->colorID->CellAttributes() ?>>
...
<?php if ($used_cars->bodyConditionID->Visible) { // bodyConditionID ?>
	<div id="r_bodyConditionID" class="form-group">
		<label id="elh_used_cars_bodyConditionID" for="x_bodyConditionID" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->bodyConditionID->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->bodyConditionID->CellAttributes() ?>>
...
<?php if ($used_cars->summary->Visible) { // summary ?>
	<div id="r_summary" class="form-group">
		<label id="elh_used_cars_summary" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->summary->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->summary->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fused_carsedit", "x_summary", 35, 4, <?php echo ($used_cars->summary->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if ($used_cars->term->Visible) { // term ?>
	<div id="r_term" class="form-group">
		<label id="elh_used_cars_term" for="x_term" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->term->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->term->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $used_cars_edit->MultiPages->PageStyle("3") ?>" id="tab_used_cars3"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($used_cars->_thumbnail->Visible) { // thumbnail ?>
	<div id="r__thumbnail" class="form-group">
		<label id="elh_used_cars__thumbnail" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->_thumbnail->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->_thumbnail->CellAttributes() ?>>
...
<?php if ($used_cars->img_01->Visible) { // img_01 ?>
	<div id="r_img_01" class="form-group">
		<label id="elh_used_cars_img_01" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_01->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_01->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_01"] == "0") { ?>
<input type="hidden" name="fa_x_img_01" id= "fa_x_img_01" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_01" id= "fa_x_img_01" value="1">
<?php } ?>
...
<?php if ($used_cars->img_02->Visible) { // img_02 ?>
	<div id="r_img_02" class="form-group">
		<label id="elh_used_cars_img_02" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_02->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_02->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_02"] == "0") { ?>
<input type="hidden" name="fa_x_img_02" id= "fa_x_img_02" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_02" id= "fa_x_img_02" value="1">
<?php } ?>
...
<?php if ($used_cars->img_03->Visible) { // img_03 ?>
	<div id="r_img_03" class="form-group">
		<label id="elh_used_cars_img_03" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_03->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_03->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_03"] == "0") { ?>
<input type="hidden" name="fa_x_img_03" id= "fa_x_img_03" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_03" id= "fa_x_img_03" value="1">
<?php } ?>
...
<?php if ($used_cars->img_04->Visible) { // img_04 ?>
	<div id="r_img_04" class="form-group">
		<label id="elh_used_cars_img_04" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_04->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_04->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_04"] == "0") { ?>
<input type="hidden" name="fa_x_img_04" id= "fa_x_img_04" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_04" id= "fa_x_img_04" value="1">
<?php } ?>
...
<?php if ($used_cars->img_05->Visible) { // img_05 ?>
	<div id="r_img_05" class="form-group">
		<label id="elh_used_cars_img_05" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_05->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_05->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_05"] == "0") { ?>
<input type="hidden" name="fa_x_img_05" id= "fa_x_img_05" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_05" id= "fa_x_img_05" value="1">
<?php } ?>
...
<?php if ($used_cars->img_06->Visible) { // img_06 ?>
	<div id="r_img_06" class="form-group">
		<label id="elh_used_cars_img_06" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_06->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_06->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_06"] == "0") { ?>
<input type="hidden" name="fa_x_img_06" id= "fa_x_img_06" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_06" id= "fa_x_img_06" value="1">
<?php } ?>
...
<?php if ($used_cars->img_07->Visible) { // img_07 ?>
	<div id="r_img_07" class="form-group">
		<label id="elh_used_cars_img_07" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_07->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_07->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_07"] == "0") { ?>
<input type="hidden" name="fa_x_img_07" id= "fa_x_img_07" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_07" id= "fa_x_img_07" value="1">
<?php } ?>
...
<?php if ($used_cars->img_08->Visible) { // img_08 ?>
	<div id="r_img_08" class="form-group">
		<label id="elh_used_cars_img_08" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_08->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_08->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_08"] == "0") { ?>
<input type="hidden" name="fa_x_img_08" id= "fa_x_img_08" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_08" id= "fa_x_img_08" value="1">
<?php } ?>
...
<?php if ($used_cars->img_09->Visible) { // img_09 ?>
	<div id="r_img_09" class="form-group">
		<label id="elh_used_cars_img_09" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_09->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_09->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_09"] == "0") { ?>
<input type="hidden" name="fa_x_img_09" id= "fa_x_img_09" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_09" id= "fa_x_img_09" value="1">
<?php } ?>
...
<?php if ($used_cars->img_10->Visible) { // img_10 ?>
	<div id="r_img_10" class="form-group">
		<label id="elh_used_cars_img_10" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_10->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_10->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_10"] == "0") { ?>
<input type="hidden" name="fa_x_img_10" id= "fa_x_img_10" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_10" id= "fa_x_img_10" value="1">
<?php } ?>
...
<?php if ($used_cars->img_11->Visible) { // img_11 ?>
	<div id="r_img_11" class="form-group">
		<label id="elh_used_cars_img_11" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_11->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_11->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_11"] == "0") { ?>
<input type="hidden" name="fa_x_img_11" id= "fa_x_img_11" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_11" id= "fa_x_img_11" value="1">
<?php } ?>
...
<?php if ($used_cars->img_12->Visible) { // img_12 ?>
	<div id="r_img_12" class="form-group">
		<label id="elh_used_cars_img_12" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->img_12->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->img_12->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_img_12"] == "0") { ?>
<input type="hidden" name="fa_x_img_12" id= "fa_x_img_12" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_12" id= "fa_x_img_12" value="1">
<?php } ?>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $used_cars_edit->MultiPages->PageStyle("4") ?>" id="tab_used_cars4"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($used_cars->extra_features->Visible) { // extra_features ?>
	<div id="r_extra_features" class="form-group">
		<label id="elh_used_cars_extra_features" class="<?php echo $used_cars_edit->LeftColumnClass ?>"><?php echo $used_cars->extra_features->FldCaption() ?></label>
		<div class="<?php echo $used_cars_edit->RightColumnClass ?>"><div<?php echo $used_cars->extra_features->CellAttributes() ?>>
...
<?php if (!$used_cars_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $used_cars_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $used_cars_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
