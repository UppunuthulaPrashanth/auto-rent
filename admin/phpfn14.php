...
function &DbHelper($dbid = "") {
	$dbclass = "cgravityalpha_autorent_db";
...
function CurrentProjectID() {
	...
	return "{F745D41A-D8C3-4E57-B065-3A7F49376673}";
}
...
function ew_GetFileUploadUrl($fld, $val, $resize = FALSE) {
	if (!ew_EmptyStr($val)) {
		...
		if ($encrypt) {
			...
			$fn = "ewfile14.php?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_Encrypt($path . $val) .
...
class cExportWord2 extends cExportBase {
	var $phpword, $section, $cellwidth, $styleTable, $phpwordTbl, $rowtype;
	static $MaxImageWidth = 250; // Max image width

	// Constructor
	function __construct(&$tbl, $style) {
		global $EW_EXPORT_WORD_CELL_WIDTH;
		parent::__construct($tbl, $style);
	...
	private function ConvertToUtf8($value) {
		return ew_ConvertToUtf8(html_entity_decode($value));
	}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setCurrentUserName($rs->fields('name')); // Load user name
