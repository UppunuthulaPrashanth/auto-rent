...
$mtr_transmission = NULL;

//
// Table class for mtr_transmission
//
class cmtr_transmission extends cTable {
	var $transmissionID;
	var $transmission;
	var $so;
	var $active;
	...
	function __construct() {
		...
		$this->TableVar = 'mtr_transmission';
		$this->TableName = 'mtr_transmission';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`mtr_transmission`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// transmissionID
		$this->transmissionID = new cField('mtr_transmission', 'mtr_transmission', 'x_transmissionID', 'transmissionID', '`transmissionID`', '`transmissionID`', 3, -1, FALSE, '`transmissionID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// transmission
		$this->transmission = new cField('mtr_transmission', 'mtr_transmission', 'x_transmission', 'transmission', '`transmission`', '`transmission`', 200, -1, FALSE, '`transmission`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->transmission->Sortable = TRUE; // Allow sort
		$this->fields['transmission'] = &$this->transmission;

		// so
		$this->so = new cField('mtr_transmission', 'mtr_transmission', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('mtr_transmission', 'mtr_transmission', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`mtr_transmission`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->transmissionID->setDbValue($conn->Insert_ID());
			$rs['transmissionID'] = $this->transmissionID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('transmissionID', $rs))
				ew_AddFilter($where, ew_QuotedName('transmissionID', $this->DBID) . '=' . ew_QuotedValue($rs['transmissionID'], $this->transmissionID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`transmissionID` = @transmissionID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->transmissionID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->transmissionID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@transmissionID@", ew_AdjustSql($this->transmissionID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "mtr_transmissionlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "mtr_transmissionedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "mtr_transmissionadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("mtr_transmissionview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("mtr_transmissionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "mtr_transmissionadd.php?" . $this->UrlParm($parm);
		else
			$url = "mtr_transmissionadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("mtr_transmissionedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("mtr_transmissionadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("mtr_transmissiondelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "transmissionID:" . ew_VarToJson($this->transmissionID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->transmissionID->CurrentValue)) {
			$sUrl .= "transmissionID=" . urlencode($this->transmissionID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["transmissionID"]))
				$arKeys[] = $_POST["transmissionID"];
			elseif (isset($_GET["transmissionID"]))
				$arKeys[] = $_GET["transmissionID"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->transmissionID->CurrentValue = $key;
