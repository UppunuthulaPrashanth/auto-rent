...
class cusers_add extends cusers {
	...
	function LoadDefaultValues() {
		...
		$this->emiratesID->CurrentValue = NULL;
		$this->emiratesID->OldValue = $this->emiratesID->CurrentValue;
		$this->visaStatus->CurrentValue = NULL;
		$this->visaStatus->OldValue = $this->visaStatus->CurrentValue;
		$this->licenseNumber->CurrentValue = NULL;
		$this->licenseNumber->OldValue = $this->licenseNumber->CurrentValue;
		$this->licenseExpiry->CurrentValue = NULL;
		$this->licenseExpiry->OldValue = $this->licenseExpiry->CurrentValue;
		$this->licensePlaceOfIssue->CurrentValue = NULL;
		$this->licensePlaceOfIssue->OldValue = $this->licensePlaceOfIssue->CurrentValue;
		$this->licenseAttachment->Upload->DbValue = NULL;
		$this->licenseAttachment->OldValue = $this->licenseAttachment->Upload->DbValue;
		$this->licenseAttachment->CurrentValue = NULL; // Clear file related field
		$this->passportNumber->CurrentValue = NULL;
		$this->passportNumber->OldValue = $this->passportNumber->CurrentValue;
		$this->passportExpiry->CurrentValue = NULL;
		$this->passportExpiry->OldValue = $this->passportExpiry->CurrentValue;
		$this->passportPlaceOfIssue->CurrentValue = NULL;
		$this->passportPlaceOfIssue->OldValue = $this->passportPlaceOfIssue->CurrentValue;
		$this->passportAttachment->Upload->DbValue = NULL;
		$this->passportAttachment->OldValue = $this->passportAttachment->Upload->DbValue;
		$this->passportAttachment->CurrentValue = NULL; // Clear file related field
		$this->signUpNewsletter->CurrentValue = NULL;
		$this->signUpNewsletter->OldValue = $this->signUpNewsletter->CurrentValue;
		$this->password->CurrentValue = NULL;
		$this->password->OldValue = $this->password->CurrentValue;
		$this->currentCurrency->CurrentValue = NULL;
		$this->currentCurrency->OldValue = $this->currentCurrency->CurrentValue;
		$this->currentLanguage->CurrentValue = NULL;
		$this->currentLanguage->OldValue = $this->currentLanguage->CurrentValue;
	...
	function NewRow() {
		...
		$row['emiratesID'] = $this->emiratesID->CurrentValue;
		$row['visaStatus'] = $this->visaStatus->CurrentValue;
		$row['licenseNumber'] = $this->licenseNumber->CurrentValue;
		$row['licenseExpiry'] = $this->licenseExpiry->CurrentValue;
		$row['licensePlaceOfIssue'] = $this->licensePlaceOfIssue->CurrentValue;
		$row['licenseAttachment'] = $this->licenseAttachment->Upload->DbValue;
		$row['passportNumber'] = $this->passportNumber->CurrentValue;
		$row['passportExpiry'] = $this->passportExpiry->CurrentValue;
		$row['passportPlaceOfIssue'] = $this->passportPlaceOfIssue->CurrentValue;
		$row['passportAttachment'] = $this->passportAttachment->Upload->DbValue;
		$row['signUpNewsletter'] = $this->signUpNewsletter->CurrentValue;
		$row['password'] = $this->password->CurrentValue;
		$row['currentCurrency'] = $this->currentCurrency->CurrentValue;
		$row['currentLanguage'] = $this->currentLanguage->CurrentValue;
	...
	function RenderRow() {
		...
		$this->createdDate->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->licenseAttachment);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->passportAttachment);
	...
	function AddRow($rsold = NULL) {
		...
		// emiratesID
		$this->emiratesID->SetDbValueDef($rsnew, $this->emiratesID->CurrentValue, NULL, FALSE);

		// visaStatus
		$this->visaStatus->SetDbValueDef($rsnew, $this->visaStatus->CurrentValue, NULL, FALSE);

		// licenseNumber
		$this->licenseNumber->SetDbValueDef($rsnew, $this->licenseNumber->CurrentValue, NULL, FALSE);

		// licenseExpiry
		$this->licenseExpiry->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->licenseExpiry->CurrentValue, 7), NULL, FALSE);

		// licensePlaceOfIssue
		$this->licensePlaceOfIssue->SetDbValueDef($rsnew, $this->licensePlaceOfIssue->CurrentValue, NULL, FALSE);

		// licenseAttachment
		if ($this->licenseAttachment->Visible && !$this->licenseAttachment->Upload->KeepFile) {
			$this->licenseAttachment->Upload->DbValue = ""; // No need to delete old file
		...
		// passportExpiry
		$this->passportExpiry->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->passportExpiry->CurrentValue, 7), NULL, FALSE);

		// passportPlaceOfIssue
		$this->passportPlaceOfIssue->SetDbValueDef($rsnew, $this->passportPlaceOfIssue->CurrentValue, NULL, FALSE);

		// passportAttachment
		if ($this->passportAttachment->Visible && !$this->passportAttachment->Upload->KeepFile) {
			$this->passportAttachment->Upload->DbValue = ""; // No need to delete old file
		...
		// signUpNewsletter
		$this->signUpNewsletter->SetDbValueDef($rsnew, $this->signUpNewsletter->CurrentValue, NULL, FALSE);

		// password
		$this->password->SetDbValueDef($rsnew, $this->password->CurrentValue, NULL, FALSE);

		// currentCurrency
		$this->currentCurrency->SetDbValueDef($rsnew, $this->currentCurrency->CurrentValue, NULL, FALSE);

		// currentLanguage
		$this->currentLanguage->SetDbValueDef($rsnew, $this->currentLanguage->CurrentValue, NULL, FALSE);
		...
		if ($this->licenseAttachment->Visible && !$this->licenseAttachment->Upload->KeepFile) {
			...
			if (!ew_Empty($this->licenseAttachment->Upload->FileName)) {
				...
				$this->licenseAttachment->SetDbValueDef($rsnew, $this->licenseAttachment->Upload->FileName, NULL, FALSE);
			}
		}
		if ($this->passportAttachment->Visible && !$this->passportAttachment->Upload->KeepFile) {
			$this->passportAttachment->UploadPath = 'uploads/documents';
			...
			if (!ew_Empty($this->passportAttachment->Upload->FileName)) {
				...
				$this->passportAttachment->SetDbValueDef($rsnew, $this->passportAttachment->Upload->FileName, NULL, FALSE);
			}
		}
...
fusersadd.Lists["x_nationality"] = {"LinkField":"x_nationalityID","Ajax":true,"AutoFill":false,"DisplayFields":["x_nationalityName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_nationality"};
fusersadd.Lists["x_nationality"].Data = "<?php echo $users_add->nationality->LookupFilterQuery(FALSE, "add") ?>";
fusersadd.Lists["x_signUpNewsletter"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fusersadd.Lists["x_signUpNewsletter"].Options = <?php echo json_encode($users_add->signUpNewsletter->Options()) ?>;
...
<?php if ($users->firstName->Visible) { // firstName ?>
	<div id="r_firstName" class="form-group">
		<label id="elh_users_firstName" for="x_firstName" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->firstName->FldCaption() ?></label>
...
<?php if ($users->lastName->Visible) { // lastName ?>
	<div id="r_lastName" class="form-group">
		<label id="elh_users_lastName" for="x_lastName" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->lastName->FldCaption() ?></label>
...
<?php if ($users->emailID->Visible) { // emailID ?>
	<div id="r_emailID" class="form-group">
		<label id="elh_users_emailID" for="x_emailID" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->emailID->FldCaption() ?></label>
...
<?php if ($users->mobileNo->Visible) { // mobileNo ?>
	<div id="r_mobileNo" class="form-group">
		<label id="elh_users_mobileNo" for="x_mobileNo" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->mobileNo->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->mobileNo->CellAttributes() ?>>
...
<?php if ($users->country->Visible) { // country ?>
	<div id="r_country" class="form-group">
		<label id="elh_users_country" for="x_country" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->country->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->country->CellAttributes() ?>>
...
<?php if ($users->address->Visible) { // address ?>
	<div id="r_address" class="form-group">
		<label id="elh_users_address" for="x_address" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->address->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->address->CellAttributes() ?>>
...
<?php if ($users->state->Visible) { // state ?>
	<div id="r_state" class="form-group">
		<label id="elh_users_state" for="x_state" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->state->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->state->CellAttributes() ?>>
...
<?php if ($users->pincode->Visible) { // pincode ?>
	<div id="r_pincode" class="form-group">
		<label id="elh_users_pincode" for="x_pincode" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->pincode->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->pincode->CellAttributes() ?>>
...
<?php if ($users->emiratesID->Visible) { // emiratesID ?>
	<div id="r_emiratesID" class="form-group">
		<label id="elh_users_emiratesID" for="x_emiratesID" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->emiratesID->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->emiratesID->CellAttributes() ?>>
...
<?php if ($users->visaStatus->Visible) { // visaStatus ?>
	<div id="r_visaStatus" class="form-group">
		<label id="elh_users_visaStatus" for="x_visaStatus" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->visaStatus->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->visaStatus->CellAttributes() ?>>
...
<?php if ($users->signUpNewsletter->Visible) { // signUpNewsletter ?>
	<div id="r_signUpNewsletter" class="form-group">
		<label id="elh_users_signUpNewsletter" for="x_signUpNewsletter" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->signUpNewsletter->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->signUpNewsletter->CellAttributes() ?>>
...
<?php if ($users->currentCurrency->Visible) { // currentCurrency ?>
	<div id="r_currentCurrency" class="form-group">
		<label id="elh_users_currentCurrency" for="x_currentCurrency" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->currentCurrency->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->currentCurrency->CellAttributes() ?>>
...
<?php if ($users->currentLanguage->Visible) { // currentLanguage ?>
	<div id="r_currentLanguage" class="form-group">
		<label id="elh_users_currentLanguage" for="x_currentLanguage" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->currentLanguage->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->currentLanguage->CellAttributes() ?>>
...
<?php if ($users->createdDate->Visible) { // createdDate ?>
	<div id="r_createdDate" class="form-group">
		<label id="elh_users_createdDate" for="x_createdDate" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->createdDate->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->createdDate->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fusersadd", "x_createdDate", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($users->licenseNumber->Visible) { // licenseNumber ?>
	<div id="r_licenseNumber" class="form-group">
		<label id="elh_users_licenseNumber" for="x_licenseNumber" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->licenseNumber->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->licenseNumber->CellAttributes() ?>>
...
<?php if ($users->licenseExpiry->Visible) { // licenseExpiry ?>
	<div id="r_licenseExpiry" class="form-group">
		<label id="elh_users_licenseExpiry" for="x_licenseExpiry" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->licenseExpiry->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->licenseExpiry->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fusersadd", "x_licenseExpiry", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($users->licensePlaceOfIssue->Visible) { // licensePlaceOfIssue ?>
	<div id="r_licensePlaceOfIssue" class="form-group">
		<label id="elh_users_licensePlaceOfIssue" for="x_licensePlaceOfIssue" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->licensePlaceOfIssue->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->licensePlaceOfIssue->CellAttributes() ?>>
...
<?php if ($users->licenseAttachment->Visible) { // licenseAttachment ?>
	<div id="r_licenseAttachment" class="form-group">
		<label id="elh_users_licenseAttachment" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->licenseAttachment->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->licenseAttachment->CellAttributes() ?>>
...
<?php if ($users->passportNumber->Visible) { // passportNumber ?>
	<div id="r_passportNumber" class="form-group">
		<label id="elh_users_passportNumber" for="x_passportNumber" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->passportNumber->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->passportNumber->CellAttributes() ?>>
...
<?php if ($users->passportExpiry->Visible) { // passportExpiry ?>
	<div id="r_passportExpiry" class="form-group">
		<label id="elh_users_passportExpiry" for="x_passportExpiry" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->passportExpiry->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->passportExpiry->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fusersadd", "x_passportExpiry", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($users->passportPlaceOfIssue->Visible) { // passportPlaceOfIssue ?>
	<div id="r_passportPlaceOfIssue" class="form-group">
		<label id="elh_users_passportPlaceOfIssue" for="x_passportPlaceOfIssue" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->passportPlaceOfIssue->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->passportPlaceOfIssue->CellAttributes() ?>>
...
<?php if ($users->passportAttachment->Visible) { // passportAttachment ?>
	<div id="r_passportAttachment" class="form-group">
		<label id="elh_users_passportAttachment" class="<?php echo $users_add->LeftColumnClass ?>"><?php echo $users->passportAttachment->FldCaption() ?></label>
		<div class="<?php echo $users_add->RightColumnClass ?>"><div<?php echo $users->passportAttachment->CellAttributes() ?>>
