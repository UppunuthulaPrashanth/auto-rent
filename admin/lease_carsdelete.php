...
$lease_cars_delete = NULL; // Initialize page object first

class clease_cars_delete extends clease_cars {
	...
	// Page object name
	var $PageObjName = 'lease_cars_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("lease_carslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in lease_cars class, lease_carsinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("lease_carslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($lease_cars_delete)) $lease_cars_delete = new clease_cars_delete();

// Page init
$lease_cars_delete->Page_Init();

// Page main
$lease_cars_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$lease_cars_delete->Page_Render();
?>
...
var CurrentForm = flease_carsdelete = new ew_Form("flease_carsdelete", "delete");

// Form_CustomValidate event
flease_carsdelete.Form_CustomValidate = 
...
flease_carsdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
flease_carsdelete.Lists["x_makeID"] = {"LinkField":"x_makeID","Ajax":true,"AutoFill":false,"DisplayFields":["x_make","","",""],"ParentFields":[],"ChildFields":["x_modelID"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_make"};
flease_carsdelete.Lists["x_makeID"].Data = "<?php echo $lease_cars_delete->makeID->LookupFilterQuery(FALSE, "delete") ?>";
flease_carsdelete.Lists["x_modelID"] = {"LinkField":"x_modelID","Ajax":true,"AutoFill":false,"DisplayFields":["x_model","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_model"};
flease_carsdelete.Lists["x_modelID"].Data = "<?php echo $lease_cars_delete->modelID->LookupFilterQuery(FALSE, "delete") ?>";
flease_carsdelete.Lists["x_bodyTypeID"] = {"LinkField":"x_bodyTypeID","Ajax":true,"AutoFill":false,"DisplayFields":["x_bodytype","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_bodytype"};
flease_carsdelete.Lists["x_bodyTypeID"].Data = "<?php echo $lease_cars_delete->bodyTypeID->LookupFilterQuery(FALSE, "delete") ?>";
flease_carsdelete.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
flease_carsdelete.Lists["x_active"].Options = <?php echo json_encode($lease_cars_delete->active->Options()) ?>;
...
<?php $lease_cars_delete->ShowPageHeader(); ?>
<?php
$lease_cars_delete->ShowMessage();
?>
<form name="flease_carsdelete" id="flease_carsdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($lease_cars_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $lease_cars_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="lease_cars">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($lease_cars_delete->RecKeys as $key) { ?>
...
<?php if ($lease_cars->leaseCarID->Visible) { // leaseCarID ?>
		<th class="<?php echo $lease_cars->leaseCarID->HeaderCellClass() ?>"><span id="elh_lease_cars_leaseCarID" class="lease_cars_leaseCarID"><?php echo $lease_cars->leaseCarID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($lease_cars->makeID->Visible) { // makeID ?>
		<th class="<?php echo $lease_cars->makeID->HeaderCellClass() ?>"><span id="elh_lease_cars_makeID" class="lease_cars_makeID"><?php echo $lease_cars->makeID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($lease_cars->modelID->Visible) { // modelID ?>
		<th class="<?php echo $lease_cars->modelID->HeaderCellClass() ?>"><span id="elh_lease_cars_modelID" class="lease_cars_modelID"><?php echo $lease_cars->modelID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($lease_cars->bodyTypeID->Visible) { // bodyTypeID ?>
		<th class="<?php echo $lease_cars->bodyTypeID->HeaderCellClass() ?>"><span id="elh_lease_cars_bodyTypeID" class="lease_cars_bodyTypeID"><?php echo $lease_cars->bodyTypeID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($lease_cars->image->Visible) { // image ?>
		<th class="<?php echo $lease_cars->image->HeaderCellClass() ?>"><span id="elh_lease_cars_image" class="lease_cars_image"><?php echo $lease_cars->image->FldCaption() ?></span></th>
<?php } ?>
<?php if ($lease_cars->monthlyAED->Visible) { // monthlyAED ?>
		<th class="<?php echo $lease_cars->monthlyAED->HeaderCellClass() ?>"><span id="elh_lease_cars_monthlyAED" class="lease_cars_monthlyAED"><?php echo $lease_cars->monthlyAED->FldCaption() ?></span></th>
<?php } ?>
<?php if ($lease_cars->active->Visible) { // active ?>
		<th class="<?php echo $lease_cars->active->HeaderCellClass() ?>"><span id="elh_lease_cars_active" class="lease_cars_active"><?php echo $lease_cars->active->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$lease_cars_delete->RecCnt = 0;
$i = 0;
while (!$lease_cars_delete->Recordset->EOF) {
	$lease_cars_delete->RecCnt++;
	$lease_cars_delete->RowCnt++;

	// Set row properties
	$lease_cars->ResetAttrs();
	$lease_cars->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$lease_cars_delete->LoadRowValues($lease_cars_delete->Recordset);

	// Render row
	$lease_cars_delete->RenderRow();
?>
	<tr<?php echo $lease_cars->RowAttributes() ?>>
<?php if ($lease_cars->leaseCarID->Visible) { // leaseCarID ?>
		<td<?php echo $lease_cars->leaseCarID->CellAttributes() ?>>
<span id="el<?php echo $lease_cars_delete->RowCnt ?>_lease_cars_leaseCarID" class="lease_cars_leaseCarID">
...
<?php } ?>
<?php if ($lease_cars->makeID->Visible) { // makeID ?>
		<td<?php echo $lease_cars->makeID->CellAttributes() ?>>
<span id="el<?php echo $lease_cars_delete->RowCnt ?>_lease_cars_makeID" class="lease_cars_makeID">
...
<?php } ?>
<?php if ($lease_cars->modelID->Visible) { // modelID ?>
		<td<?php echo $lease_cars->modelID->CellAttributes() ?>>
<span id="el<?php echo $lease_cars_delete->RowCnt ?>_lease_cars_modelID" class="lease_cars_modelID">
...
<?php } ?>
<?php if ($lease_cars->bodyTypeID->Visible) { // bodyTypeID ?>
		<td<?php echo $lease_cars->bodyTypeID->CellAttributes() ?>>
<span id="el<?php echo $lease_cars_delete->RowCnt ?>_lease_cars_bodyTypeID" class="lease_cars_bodyTypeID">
...
<?php } ?>
<?php if ($lease_cars->image->Visible) { // image ?>
		<td<?php echo $lease_cars->image->CellAttributes() ?>>
<span id="el<?php echo $lease_cars_delete->RowCnt ?>_lease_cars_image" class="lease_cars_image">
<span>
...
<?php } ?>
<?php if ($lease_cars->monthlyAED->Visible) { // monthlyAED ?>
		<td<?php echo $lease_cars->monthlyAED->CellAttributes() ?>>
<span id="el<?php echo $lease_cars_delete->RowCnt ?>_lease_cars_monthlyAED" class="lease_cars_monthlyAED">
...
<?php } ?>
<?php if ($lease_cars->active->Visible) { // active ?>
		<td<?php echo $lease_cars->active->CellAttributes() ?>>
<span id="el<?php echo $lease_cars_delete->RowCnt ?>_lease_cars_active" class="lease_cars_active">
...
<?php
	$lease_cars_delete->Recordset->MoveNext();
}
$lease_cars_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $lease_cars_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
flease_carsdelete.Init();
</script>
<?php
$lease_cars_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$lease_cars_delete->Page_Terminate();
?>
