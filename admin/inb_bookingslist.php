...
$inb_bookings_list = NULL; // Initialize page object first

class cinb_bookings_list extends cinb_bookings {
	...
	// Page object name
	var $PageObjName = 'inb_bookings_list';

	// Grid form hidden field names
	var $FormName = 'finb_bookingslist';
	...
	function __construct() {
		...
		$this->AddUrl = "inb_bookingsadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "inb_bookingsdelete.php";
		$this->MultiUpdateUrl = "inb_bookingsupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption finb_bookingslistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "finb_bookingslistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "finb_bookingslistsrch", $filters);
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->rentLeaseCarID, $Default, FALSE); // rentLeaseCarID
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->rentLeaseCarID->AdvancedSearch->Save(); // rentLeaseCarID
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->pickUpLocation, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->dropLocation, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->pickUpDate, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->dropDate, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->bookingTerm, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->gps, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->totalAmount, $arKeywords, $type);
	...
	function CheckSearchParms() {
		...
		if ($this->_userID->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->rentLeaseCarID->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		...
		$this->rentLeaseCarID->AdvancedSearch->UnsetSession();
	...
	function RestoreSearchParms() {
		...
		$this->rentLeaseCarID->AdvancedSearch->Load();
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"finb_bookingslistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"finb_bookingslistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.finb_bookingslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"finb_bookingslistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadSearchValues() {
		...
		// rentLeaseCarID
		$this->rentLeaseCarID->AdvancedSearch->SearchValue = @$_GET["x_rentLeaseCarID"];
		if ($this->rentLeaseCarID->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->rentLeaseCarID->AdvancedSearch->SearchOperator = @$_GET["z_rentLeaseCarID"];
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_inb_bookings\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_inb_bookings',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.finb_bookingslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportQueryString() {
		...
		$this->AddSearchQueryString($sQry, $this->rentLeaseCarID); // rentLeaseCarID
...
// Create page object
if (!isset($inb_bookings_list)) $inb_bookings_list = new cinb_bookings_list();

// Page init
$inb_bookings_list->Page_Init();

// Page main
$inb_bookings_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$inb_bookings_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($inb_bookings->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = finb_bookingslist = new ew_Form("finb_bookingslist", "list");
finb_bookingslist.FormKeyCountName = '<?php echo $inb_bookings_list->FormKeyCountName ?>';

// Form_CustomValidate event
finb_bookingslist.Form_CustomValidate = 
...
finb_bookingslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finb_bookingslist.Lists["x__userID"] = {"LinkField":"x__userID","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstName","x_lastName","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
finb_bookingslist.Lists["x__userID"].Data = "<?php echo $inb_bookings_list->_userID->LookupFilterQuery(FALSE, "list") ?>";
finb_bookingslist.Lists["x_rentLeaseCarID"] = {"LinkField":"x_rentLeaseCarID","Ajax":true,"AutoFill":false,"DisplayFields":["x_carTitle","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"rent_lease_cars"};
finb_bookingslist.Lists["x_rentLeaseCarID"].Data = "<?php echo $inb_bookings_list->rentLeaseCarID->LookupFilterQuery(FALSE, "list") ?>";
finb_bookingslist.Lists["x_pickUpLocation"] = {"LinkField":"x_pdLocationID","Ajax":true,"AutoFill":false,"DisplayFields":["x_location","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pickup_drop_locations"};
finb_bookingslist.Lists["x_pickUpLocation"].Data = "<?php echo $inb_bookings_list->pickUpLocation->LookupFilterQuery(FALSE, "list") ?>";
finb_bookingslist.Lists["x_dropLocation"] = {"LinkField":"x_pdLocationID","Ajax":true,"AutoFill":false,"DisplayFields":["x_location","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pickup_drop_locations"};
finb_bookingslist.Lists["x_dropLocation"].Data = "<?php echo $inb_bookings_list->dropLocation->LookupFilterQuery(FALSE, "list") ?>";
finb_bookingslist.Lists["x_paymentMethod"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finb_bookingslist.Lists["x_paymentMethod"].Options = <?php echo json_encode($inb_bookings_list->paymentMethod->Options()) ?>;

// Form object for search
var CurrentSearchForm = finb_bookingslistsrch = new ew_Form("finb_bookingslistsrch");

// Validate function for search
finb_bookingslistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
finb_bookingslistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
finb_bookingslistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finb_bookingslistsrch.Lists["x_paymentMethod"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finb_bookingslistsrch.Lists["x_paymentMethod"].Options = <?php echo json_encode($inb_bookings_list->paymentMethod->Options()) ?>;
</script>
<style type="text/css">
...
<?php if ($inb_bookings->Export == "") { ?>
<div class="ewToolbar">
<?php if ($inb_bookings_list->TotalRecs > 0 && $inb_bookings_list->ExportOptions->Visible()) { ?>
<?php $inb_bookings_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($inb_bookings_list->SearchOptions->Visible()) { ?>
<?php $inb_bookings_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($inb_bookings_list->FilterOptions->Visible()) { ?>
<?php $inb_bookings_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $inb_bookings_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($inb_bookings_list->TotalRecs <= 0)
			$inb_bookings_list->TotalRecs = $inb_bookings->ListRecordCount();
	} else {
		if (!$inb_bookings_list->Recordset && ($inb_bookings_list->Recordset = $inb_bookings_list->LoadRecordset()))
			$inb_bookings_list->TotalRecs = $inb_bookings_list->Recordset->RecordCount();
	}
	$inb_bookings_list->StartRec = 1;
	if ($inb_bookings_list->DisplayRecs <= 0 || ($inb_bookings->Export <> "" && $inb_bookings->ExportAll)) // Display all records
		$inb_bookings_list->DisplayRecs = $inb_bookings_list->TotalRecs;
	if (!($inb_bookings->Export <> "" && $inb_bookings->ExportAll))
		$inb_bookings_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$inb_bookings_list->Recordset = $inb_bookings_list->LoadRecordset($inb_bookings_list->StartRec-1, $inb_bookings_list->DisplayRecs);

	// Set no record found message
	if ($inb_bookings->CurrentAction == "" && $inb_bookings_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$inb_bookings_list->setWarningMessage(ew_DeniedMsg());
		if ($inb_bookings_list->SearchWhere == "0=101")
			$inb_bookings_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$inb_bookings_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$inb_bookings_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($inb_bookings->Export == "" && $inb_bookings->CurrentAction == "") { ?>
<form name="finb_bookingslistsrch" id="finb_bookingslistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($inb_bookings_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="finb_bookingslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="inb_bookings">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$inb_bookings_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$inb_bookings->RowType = EW_ROWTYPE_SEARCH;

// Render row
$inb_bookings->ResetAttrs();
$inb_bookings_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($inb_bookings->paymentMethod->Visible) { // paymentMethod ?>
	<div id="xsc_paymentMethod" class="ewCell form-group">
		<label class="ewSearchCaption ewLabel"><?php echo $inb_bookings->paymentMethod->FldCaption() ?></label>
...
<div id="tp_x_paymentMethod" class="ewTemplate"><input type="radio" data-table="inb_bookings" data-field="x_paymentMethod" data-value-separator="<?php echo $inb_bookings->paymentMethod->DisplayValueSeparatorAttribute() ?>" name="x_paymentMethod" id="x_paymentMethod" value="{value}"<?php echo $inb_bookings->paymentMethod->EditAttributes() ?>></div>
<div id="dsl_x_paymentMethod" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $inb_bookings->paymentMethod->RadioButtonListHtml(FALSE, "x_paymentMethod") ?>
</div></div>
</span>
	</div>
<?php } ?>
...
<div id="xsr_2" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($inb_bookings_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($inb_bookings_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $inb_bookings_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($inb_bookings_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($inb_bookings_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($inb_bookings_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($inb_bookings_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $inb_bookings_list->ShowPageHeader(); ?>
<?php
$inb_bookings_list->ShowMessage();
?>
<?php if ($inb_bookings_list->TotalRecs > 0 || $inb_bookings->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($inb_bookings_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> inb_bookings">
<?php if ($inb_bookings->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($inb_bookings->CurrentAction <> "gridadd" && $inb_bookings->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($inb_bookings_list->Pager)) $inb_bookings_list->Pager = new cPrevNextPager($inb_bookings_list->StartRec, $inb_bookings_list->DisplayRecs, $inb_bookings_list->TotalRecs, $inb_bookings_list->AutoHidePager) ?>
<?php if ($inb_bookings_list->Pager->RecordCount > 0 && $inb_bookings_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($inb_bookings_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $inb_bookings_list->PageUrl() ?>start=<?php echo $inb_bookings_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($inb_bookings_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $inb_bookings_list->PageUrl() ?>start=<?php echo $inb_bookings_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $inb_bookings_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($inb_bookings_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $inb_bookings_list->PageUrl() ?>start=<?php echo $inb_bookings_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($inb_bookings_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $inb_bookings_list->PageUrl() ?>start=<?php echo $inb_bookings_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $inb_bookings_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $inb_bookings_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $inb_bookings_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $inb_bookings_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($inb_bookings_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="finb_bookingslist" id="finb_bookingslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($inb_bookings_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $inb_bookings_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="inb_bookings">
<div id="gmp_inb_bookings" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($inb_bookings_list->TotalRecs > 0 || $inb_bookings->CurrentAction == "gridedit") { ?>
<table id="tbl_inb_bookingslist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$inb_bookings_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$inb_bookings_list->RenderListOptions();

// Render list options (header, left)
$inb_bookings_list->ListOptions->Render("header", "left");
?>
<?php if ($inb_bookings->bookingID->Visible) { // bookingID ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->bookingID) == "") { ?>
		<th data-name="bookingID" class="<?php echo $inb_bookings->bookingID->HeaderCellClass() ?>"><div id="elh_inb_bookings_bookingID" class="inb_bookings_bookingID"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->bookingID->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="bookingID" class="<?php echo $inb_bookings->bookingID->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->bookingID) ?>',1);"><div id="elh_inb_bookings_bookingID" class="inb_bookings_bookingID">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->bookingID->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->bookingID->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->bookingID->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->bookingNumber->Visible) { // bookingNumber ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->bookingNumber) == "") { ?>
		<th data-name="bookingNumber" class="<?php echo $inb_bookings->bookingNumber->HeaderCellClass() ?>"><div id="elh_inb_bookings_bookingNumber" class="inb_bookings_bookingNumber"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->bookingNumber->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="bookingNumber" class="<?php echo $inb_bookings->bookingNumber->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->bookingNumber) ?>',1);"><div id="elh_inb_bookings_bookingNumber" class="inb_bookings_bookingNumber">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->bookingNumber->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->bookingNumber->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->bookingNumber->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->_userID->Visible) { // userID ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->_userID) == "") { ?>
		<th data-name="_userID" class="<?php echo $inb_bookings->_userID->HeaderCellClass() ?>"><div id="elh_inb_bookings__userID" class="inb_bookings__userID"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->_userID->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="_userID" class="<?php echo $inb_bookings->_userID->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->_userID) ?>',1);"><div id="elh_inb_bookings__userID" class="inb_bookings__userID">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->_userID->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->_userID->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->_userID->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->rentLeaseCarID->Visible) { // rentLeaseCarID ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->rentLeaseCarID) == "") { ?>
		<th data-name="rentLeaseCarID" class="<?php echo $inb_bookings->rentLeaseCarID->HeaderCellClass() ?>"><div id="elh_inb_bookings_rentLeaseCarID" class="inb_bookings_rentLeaseCarID"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->rentLeaseCarID->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="rentLeaseCarID" class="<?php echo $inb_bookings->rentLeaseCarID->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->rentLeaseCarID) ?>',1);"><div id="elh_inb_bookings_rentLeaseCarID" class="inb_bookings_rentLeaseCarID">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->rentLeaseCarID->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->rentLeaseCarID->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->rentLeaseCarID->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->pickUpLocation->Visible) { // pickUpLocation ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->pickUpLocation) == "") { ?>
		<th data-name="pickUpLocation" class="<?php echo $inb_bookings->pickUpLocation->HeaderCellClass() ?>"><div id="elh_inb_bookings_pickUpLocation" class="inb_bookings_pickUpLocation"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->pickUpLocation->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="pickUpLocation" class="<?php echo $inb_bookings->pickUpLocation->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->pickUpLocation) ?>',1);"><div id="elh_inb_bookings_pickUpLocation" class="inb_bookings_pickUpLocation">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->pickUpLocation->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->pickUpLocation->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->pickUpLocation->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->dropLocation->Visible) { // dropLocation ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->dropLocation) == "") { ?>
		<th data-name="dropLocation" class="<?php echo $inb_bookings->dropLocation->HeaderCellClass() ?>"><div id="elh_inb_bookings_dropLocation" class="inb_bookings_dropLocation"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->dropLocation->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="dropLocation" class="<?php echo $inb_bookings->dropLocation->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->dropLocation) ?>',1);"><div id="elh_inb_bookings_dropLocation" class="inb_bookings_dropLocation">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->dropLocation->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->dropLocation->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->dropLocation->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->pickUpDate->Visible) { // pickUpDate ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->pickUpDate) == "") { ?>
		<th data-name="pickUpDate" class="<?php echo $inb_bookings->pickUpDate->HeaderCellClass() ?>"><div id="elh_inb_bookings_pickUpDate" class="inb_bookings_pickUpDate"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->pickUpDate->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="pickUpDate" class="<?php echo $inb_bookings->pickUpDate->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->pickUpDate) ?>',1);"><div id="elh_inb_bookings_pickUpDate" class="inb_bookings_pickUpDate">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->pickUpDate->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->pickUpDate->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->pickUpDate->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->noOfDays->Visible) { // noOfDays ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->noOfDays) == "") { ?>
		<th data-name="noOfDays" class="<?php echo $inb_bookings->noOfDays->HeaderCellClass() ?>"><div id="elh_inb_bookings_noOfDays" class="inb_bookings_noOfDays"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->noOfDays->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="noOfDays" class="<?php echo $inb_bookings->noOfDays->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->noOfDays) ?>',1);"><div id="elh_inb_bookings_noOfDays" class="inb_bookings_noOfDays">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->noOfDays->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->noOfDays->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->noOfDays->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->bookingTerm->Visible) { // bookingTerm ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->bookingTerm) == "") { ?>
		<th data-name="bookingTerm" class="<?php echo $inb_bookings->bookingTerm->HeaderCellClass() ?>"><div id="elh_inb_bookings_bookingTerm" class="inb_bookings_bookingTerm"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->bookingTerm->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="bookingTerm" class="<?php echo $inb_bookings->bookingTerm->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->bookingTerm) ?>',1);"><div id="elh_inb_bookings_bookingTerm" class="inb_bookings_bookingTerm">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->bookingTerm->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->bookingTerm->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->bookingTerm->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->grandTotal->Visible) { // grandTotal ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->grandTotal) == "") { ?>
		<th data-name="grandTotal" class="<?php echo $inb_bookings->grandTotal->HeaderCellClass() ?>"><div id="elh_inb_bookings_grandTotal" class="inb_bookings_grandTotal"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->grandTotal->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="grandTotal" class="<?php echo $inb_bookings->grandTotal->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->grandTotal) ?>',1);"><div id="elh_inb_bookings_grandTotal" class="inb_bookings_grandTotal">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->grandTotal->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->grandTotal->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->grandTotal->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->totalAmount->Visible) { // totalAmount ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->totalAmount) == "") { ?>
		<th data-name="totalAmount" class="<?php echo $inb_bookings->totalAmount->HeaderCellClass() ?>"><div id="elh_inb_bookings_totalAmount" class="inb_bookings_totalAmount"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->totalAmount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="totalAmount" class="<?php echo $inb_bookings->totalAmount->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->totalAmount) ?>',1);"><div id="elh_inb_bookings_totalAmount" class="inb_bookings_totalAmount">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->totalAmount->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->totalAmount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->totalAmount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->paymentMethod->Visible) { // paymentMethod ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->paymentMethod) == "") { ?>
		<th data-name="paymentMethod" class="<?php echo $inb_bookings->paymentMethod->HeaderCellClass() ?>"><div id="elh_inb_bookings_paymentMethod" class="inb_bookings_paymentMethod"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->paymentMethod->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="paymentMethod" class="<?php echo $inb_bookings->paymentMethod->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->paymentMethod) ?>',1);"><div id="elh_inb_bookings_paymentMethod" class="inb_bookings_paymentMethod">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->paymentMethod->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->paymentMethod->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->paymentMethod->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inb_bookings->dateCreated->Visible) { // dateCreated ?>
	<?php if ($inb_bookings->SortUrl($inb_bookings->dateCreated) == "") { ?>
		<th data-name="dateCreated" class="<?php echo $inb_bookings->dateCreated->HeaderCellClass() ?>"><div id="elh_inb_bookings_dateCreated" class="inb_bookings_dateCreated"><div class="ewTableHeaderCaption"><?php echo $inb_bookings->dateCreated->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="dateCreated" class="<?php echo $inb_bookings->dateCreated->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inb_bookings->SortUrl($inb_bookings->dateCreated) ?>',1);"><div id="elh_inb_bookings_dateCreated" class="inb_bookings_dateCreated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inb_bookings->dateCreated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inb_bookings->dateCreated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inb_bookings->dateCreated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$inb_bookings_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($inb_bookings->ExportAll && $inb_bookings->Export <> "") {
	$inb_bookings_list->StopRec = $inb_bookings_list->TotalRecs;
} else {

	// Set the last record to display
	if ($inb_bookings_list->TotalRecs > $inb_bookings_list->StartRec + $inb_bookings_list->DisplayRecs - 1)
		$inb_bookings_list->StopRec = $inb_bookings_list->StartRec + $inb_bookings_list->DisplayRecs - 1;
	else
		$inb_bookings_list->StopRec = $inb_bookings_list->TotalRecs;
}
$inb_bookings_list->RecCnt = $inb_bookings_list->StartRec - 1;
if ($inb_bookings_list->Recordset && !$inb_bookings_list->Recordset->EOF) {
	$inb_bookings_list->Recordset->MoveFirst();
	$bSelectLimit = $inb_bookings_list->UseSelectLimit;
	if (!$bSelectLimit && $inb_bookings_list->StartRec > 1)
		$inb_bookings_list->Recordset->Move($inb_bookings_list->StartRec - 1);
} elseif (!$inb_bookings->AllowAddDeleteRow && $inb_bookings_list->StopRec == 0) {
	$inb_bookings_list->StopRec = $inb_bookings->GridAddRowCount;
}

// Initialize aggregate
$inb_bookings->RowType = EW_ROWTYPE_AGGREGATEINIT;
$inb_bookings->ResetAttrs();
$inb_bookings_list->RenderRow();
while ($inb_bookings_list->RecCnt < $inb_bookings_list->StopRec) {
	$inb_bookings_list->RecCnt++;
	if (intval($inb_bookings_list->RecCnt) >= intval($inb_bookings_list->StartRec)) {
		$inb_bookings_list->RowCnt++;

		// Set up key count
		$inb_bookings_list->KeyCount = $inb_bookings_list->RowIndex;

		// Init row class and style
		$inb_bookings->ResetAttrs();
		$inb_bookings->CssClass = "";
		if ($inb_bookings->CurrentAction == "gridadd") {
		} else {
			$inb_bookings_list->LoadRowValues($inb_bookings_list->Recordset); // Load row values
		}
		$inb_bookings->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$inb_bookings->RowAttrs = array_merge($inb_bookings->RowAttrs, array('data-rowindex'=>$inb_bookings_list->RowCnt, 'id'=>'r' . $inb_bookings_list->RowCnt . '_inb_bookings', 'data-rowtype'=>$inb_bookings->RowType));
...
$inb_bookings_list->ListOptions->Render("body", "left", $inb_bookings_list->RowCnt);
?>
	<?php if ($inb_bookings->bookingID->Visible) { // bookingID ?>
		<td data-name="bookingID"<?php echo $inb_bookings->bookingID->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_bookingID" class="inb_bookings_bookingID">
...
<?php echo $inb_bookings->bookingID->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->bookingNumber->Visible) { // bookingNumber ?>
		<td data-name="bookingNumber"<?php echo $inb_bookings->bookingNumber->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_bookingNumber" class="inb_bookings_bookingNumber">
...
<?php echo $inb_bookings->bookingNumber->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->_userID->Visible) { // userID ?>
		<td data-name="_userID"<?php echo $inb_bookings->_userID->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings__userID" class="inb_bookings__userID">
...
<?php echo $inb_bookings->_userID->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->rentLeaseCarID->Visible) { // rentLeaseCarID ?>
		<td data-name="rentLeaseCarID"<?php echo $inb_bookings->rentLeaseCarID->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_rentLeaseCarID" class="inb_bookings_rentLeaseCarID">
...
<?php echo $inb_bookings->rentLeaseCarID->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->pickUpLocation->Visible) { // pickUpLocation ?>
		<td data-name="pickUpLocation"<?php echo $inb_bookings->pickUpLocation->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_pickUpLocation" class="inb_bookings_pickUpLocation">
...
<?php echo $inb_bookings->pickUpLocation->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->dropLocation->Visible) { // dropLocation ?>
		<td data-name="dropLocation"<?php echo $inb_bookings->dropLocation->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_dropLocation" class="inb_bookings_dropLocation">
...
<?php echo $inb_bookings->dropLocation->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->pickUpDate->Visible) { // pickUpDate ?>
		<td data-name="pickUpDate"<?php echo $inb_bookings->pickUpDate->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_pickUpDate" class="inb_bookings_pickUpDate">
...
<?php echo $inb_bookings->pickUpDate->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->noOfDays->Visible) { // noOfDays ?>
		<td data-name="noOfDays"<?php echo $inb_bookings->noOfDays->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_noOfDays" class="inb_bookings_noOfDays">
...
<?php echo $inb_bookings->noOfDays->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->bookingTerm->Visible) { // bookingTerm ?>
		<td data-name="bookingTerm"<?php echo $inb_bookings->bookingTerm->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_bookingTerm" class="inb_bookings_bookingTerm">
...
<?php echo $inb_bookings->bookingTerm->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->grandTotal->Visible) { // grandTotal ?>
		<td data-name="grandTotal"<?php echo $inb_bookings->grandTotal->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_grandTotal" class="inb_bookings_grandTotal">
...
<?php echo $inb_bookings->grandTotal->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->totalAmount->Visible) { // totalAmount ?>
		<td data-name="totalAmount"<?php echo $inb_bookings->totalAmount->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_totalAmount" class="inb_bookings_totalAmount">
...
<?php echo $inb_bookings->totalAmount->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->paymentMethod->Visible) { // paymentMethod ?>
		<td data-name="paymentMethod"<?php echo $inb_bookings->paymentMethod->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_paymentMethod" class="inb_bookings_paymentMethod">
...
<?php echo $inb_bookings->paymentMethod->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($inb_bookings->dateCreated->Visible) { // dateCreated ?>
		<td data-name="dateCreated"<?php echo $inb_bookings->dateCreated->CellAttributes() ?>>
<span id="el<?php echo $inb_bookings_list->RowCnt ?>_inb_bookings_dateCreated" class="inb_bookings_dateCreated">
...
$inb_bookings_list->ListOptions->Render("body", "right", $inb_bookings_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($inb_bookings->CurrentAction <> "gridadd")
		$inb_bookings_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($inb_bookings->CurrentAction == "") { ?>
...
// Close recordset
if ($inb_bookings_list->Recordset)
	$inb_bookings_list->Recordset->Close();
?>
<?php if ($inb_bookings->Export == "") { ?>
...
<?php } ?>
<?php if ($inb_bookings_list->TotalRecs == 0 && $inb_bookings->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($inb_bookings_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($inb_bookings->Export == "") { ?>
<script type="text/javascript">
finb_bookingslistsrch.FilterList = <?php echo $inb_bookings_list->GetFilterList() ?>;
finb_bookingslistsrch.Init();
finb_bookingslist.Init();
</script>
<?php } ?>
<?php
$inb_bookings_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($inb_bookings->Export == "") { ?>
...
<?php
$inb_bookings_list->Page_Terminate();
?>
