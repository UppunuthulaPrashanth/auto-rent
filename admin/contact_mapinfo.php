...
class ccontact_map extends cTable {
	...
	function __construct() {
		...
		// contactID
		$this->contactID = new cField('contact_map', 'contact_map', 'x_contactID', 'contactID', '`contactID`', '`contactID`', 3, -1, FALSE, '`contactID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->contactID->Sortable = TRUE; // Allow sort
		$this->contactID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['contactID'] = &$this->contactID;

		// title
		$this->title = new cField('contact_map', 'contact_map', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// phone
		$this->phone = new cField('contact_map', 'contact_map', 'x_phone', 'phone', '`phone`', '`phone`', 200, -1, FALSE, '`phone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->phone->Sortable = TRUE; // Allow sort
		$this->fields['phone'] = &$this->phone;

		// fax
		$this->fax = new cField('contact_map', 'contact_map', 'x_fax', 'fax', '`fax`', '`fax`', 200, -1, FALSE, '`fax`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fax->Sortable = TRUE; // Allow sort
		$this->fields['fax'] = &$this->fax;

		// email
		$this->_email = new cField('contact_map', 'contact_map', 'x__email', 'email', '`email`', '`email`', 200, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// map
		$this->map = new cField('contact_map', 'contact_map', 'x_map', 'map', '`map`', '`map`', 201, -1, FALSE, '`map`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->map->Sortable = TRUE; // Allow sort
		$this->fields['map'] = &$this->map;

		// so
		$this->so = new cField('contact_map', 'contact_map', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('contact_map', 'contact_map', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`contact_map`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->contactID->setDbValue($conn->Insert_ID());
			$rs['contactID'] = $this->contactID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('contactID', $rs))
				ew_AddFilter($where, ew_QuotedName('contactID', $this->DBID) . '=' . ew_QuotedValue($rs['contactID'], $this->contactID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->contactID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->contactID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@contactID@", ew_AdjustSql($this->contactID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "contact_mapedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "contact_mapadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("contact_mapview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("contact_mapview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "contact_mapadd.php?" . $this->UrlParm($parm);
		else
			$url = "contact_mapadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("contact_mapedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("contact_mapadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "contactID:" . ew_VarToJson($this->contactID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->contactID->CurrentValue)) {
			$sUrl .= "contactID=" . urlencode($this->contactID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["contactID"]))
				$arKeys[] = $_POST["contactID"];
			elseif (isset($_GET["contactID"]))
				$arKeys[] = $_GET["contactID"];
			else
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->contactID->Exportable) $Doc->ExportCaption($this->contactID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->contactID->Exportable) $Doc->ExportField($this->contactID);
