...
class cabout_us extends cTable {
	...
	function __construct() {
		...
		// aboutID
		$this->aboutID = new cField('about_us', 'about_us', 'x_aboutID', 'aboutID', '`aboutID`', '`aboutID`', 3, -1, FALSE, '`aboutID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->aboutID->Sortable = TRUE; // Allow sort
		$this->aboutID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['aboutID'] = &$this->aboutID;

		// title
		$this->title = new cField('about_us', 'about_us', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// summary
		$this->summary = new cField('about_us', 'about_us', 'x_summary', 'summary', '`summary`', '`summary`', 200, -1, FALSE, '`summary`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->summary->Sortable = TRUE; // Allow sort
		$this->fields['summary'] = &$this->summary;

		// image
		$this->image = new cField('about_us', 'about_us', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// so
		$this->so = new cField('about_us', 'about_us', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('about_us', 'about_us', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->aboutID->setDbValue($conn->Insert_ID());
			$rs['aboutID'] = $this->aboutID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('aboutID', $rs))
				ew_AddFilter($where, ew_QuotedName('aboutID', $this->DBID) . '=' . ew_QuotedValue($rs['aboutID'], $this->aboutID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->aboutID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->aboutID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@aboutID@", ew_AdjustSql($this->aboutID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "aboutID:" . ew_VarToJson($this->aboutID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->aboutID->CurrentValue)) {
			$sUrl .= "aboutID=" . urlencode($this->aboutID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["aboutID"]))
				$arKeys[] = $_POST["aboutID"];
			elseif (isset($_GET["aboutID"]))
				$arKeys[] = $_GET["aboutID"];
			else
	...
	function LoadListRowValues(&$rs) {
		$this->aboutID->setDbValue($rs->fields('aboutID'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->aboutID->Exportable) $Doc->ExportCaption($this->aboutID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->aboutID->Exportable) $Doc->ExportField($this->aboutID);
