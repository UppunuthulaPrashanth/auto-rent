...
class cservices_add extends cservices {
	...
	function Page_Init() {
		...
		$this->serviceSlug->SetVisibility();
	...
	function LoadDefaultValues() {
		...
		$this->serviceSlug->CurrentValue = NULL;
		$this->serviceSlug->OldValue = $this->serviceSlug->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->serviceSlug->FldIsDetailKey) {
			$this->serviceSlug->setFormValue($objForm->GetValue("x_serviceSlug"));
		}
	...
	function RestoreFormValues() {
		...
		$this->serviceSlug->CurrentValue = $this->serviceSlug->FormValue;
	...
	function NewRow() {
		...
		$row['serviceName'] = $this->serviceName->CurrentValue;
		$row['serviceSlug'] = $this->serviceSlug->CurrentValue;
	...
	function RenderRow() {
		...
		$this->active->ViewCustomAttributes = "";
			...
			if (trim(strval($this->serviceTypeID->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`serviceTypeID`" . ew_SearchString("=", $this->serviceTypeID->CurrentValue, EW_DATATYPE_NUMBER, "");
			}
			$sSqlWrk = "SELECT `serviceTypeID`, `serviceTypeName` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `mtr_service_type`";
			$sWhereWrk = "";
			$this->serviceTypeID->LookupFilters = array();
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->serviceTypeID, $sWhereWrk); // Call Lookup Selecting
			...
			$this->serviceTypeID->EditValue = $arwrk;
			...
			$this->serviceSlug->EditValue = ew_HtmlEncode($this->serviceSlug->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		// serviceTypeID
		$this->serviceTypeID->SetDbValueDef($rsnew, $this->serviceTypeID->CurrentValue, NULL, FALSE);

		// serviceName
		$this->serviceName->SetDbValueDef($rsnew, $this->serviceName->CurrentValue, NULL, FALSE);

		// serviceSlug
		$this->serviceSlug->SetDbValueDef($rsnew, $this->serviceSlug->CurrentValue, NULL, FALSE);
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		switch ($fld->FldVar) {
		case "x_serviceTypeID":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `serviceTypeID` AS `LinkFld`, `serviceTypeName` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `mtr_service_type`";
			$sWhereWrk = "";
			$fld->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "", "f0" => '`serviceTypeID` IN ({filter_value})', "t0" => "3", "fn0" => "");
			$sSqlWrk = "";
			$this->Lookup_Selecting($this->serviceTypeID, $sWhereWrk); // Call Lookup Selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			if ($sSqlWrk <> "")
...
fservicesadd.Lists["x_serviceTypeID"] = {"LinkField":"x_serviceTypeID","Ajax":true,"AutoFill":false,"DisplayFields":["x_serviceTypeName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"mtr_service_type"};
fservicesadd.Lists["x_serviceTypeID"].Data = "<?php echo $services_add->serviceTypeID->LookupFilterQuery(FALSE, "add") ?>";
...
<?php if ($services->serviceTypeID->Visible) { // serviceTypeID ?>
	<div id="r_serviceTypeID" class="form-group">
		<label id="elh_services_serviceTypeID" for="x_serviceTypeID" class="<?php echo $services_add->LeftColumnClass ?>"><?php echo $services->serviceTypeID->FldCaption() ?></label>
		<div class="<?php echo $services_add->RightColumnClass ?>"><div<?php echo $services->serviceTypeID->CellAttributes() ?>>
<span id="el_services_serviceTypeID">
<select data-table="services" data-field="x_serviceTypeID" data-value-separator="<?php echo $services->serviceTypeID->DisplayValueSeparatorAttribute() ?>" id="x_serviceTypeID" name="x_serviceTypeID"<?php echo $services->serviceTypeID->EditAttributes() ?>>
<?php echo $services->serviceTypeID->SelectOptionListHtml("x_serviceTypeID") ?>
</select>
</span>
<?php echo $services->serviceTypeID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($services->serviceName->Visible) { // serviceName ?>
	<div id="r_serviceName" class="form-group">
		<label id="elh_services_serviceName" for="x_serviceName" class="<?php echo $services_add->LeftColumnClass ?>"><?php echo $services->serviceName->FldCaption() ?></label>
		<div class="<?php echo $services_add->RightColumnClass ?>"><div<?php echo $services->serviceName->CellAttributes() ?>>
...
<?php } ?>
<?php if ($services->serviceSlug->Visible) { // serviceSlug ?>
	<div id="r_serviceSlug" class="form-group">
		<label id="elh_services_serviceSlug" for="x_serviceSlug" class="<?php echo $services_add->LeftColumnClass ?>"><?php echo $services->serviceSlug->FldCaption() ?></label>
		<div class="<?php echo $services_add->RightColumnClass ?>"><div<?php echo $services->serviceSlug->CellAttributes() ?>>
<span id="el_services_serviceSlug">
<input type="text" data-table="services" data-field="x_serviceSlug" name="x_serviceSlug" id="x_serviceSlug" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($services->serviceSlug->getPlaceHolder()) ?>" value="<?php echo $services->serviceSlug->EditValue ?>"<?php echo $services->serviceSlug->EditAttributes() ?>>
</span>
<?php echo $services->serviceSlug->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<span title="<?php echo $services->headerImage->FldTitle() ? $services->headerImage->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($services->headerImage->ReadOnly || $services->headerImage->Disabled) echo " hide"; ?>" data-trigger="hover">
	...
	<input type="file" title=" " data-table="services" data-field="x_headerImage" name="x_headerImage" id="x_headerImage"<?php echo $services->headerImage->EditAttributes() ?>>
</span>
...
<textarea data-table="services" data-field="x_description" name="x_description" id="x_description" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($services->description->getPlaceHolder()) ?>"<?php echo $services->description->EditAttributes() ?>><?php echo $services->description->EditValue ?></textarea>
<script type="text/javascript">
...
<input type="text" data-table="services" data-field="x_so" name="x_so" id="x_so" size="30" placeholder="<?php echo ew_HtmlEncode($services->so->getPlaceHolder()) ?>" value="<?php echo $services->so->EditValue ?>"<?php echo $services->so->EditAttributes() ?>>
</span>
...
<select data-table="services" data-field="x_active" data-value-separator="<?php echo $services->active->DisplayValueSeparatorAttribute() ?>" id="x_active" name="x_active"<?php echo $services->active->EditAttributes() ?>>
