...
//
// Table class for mtr_locations
//
class cmtr_locations extends cTable {
	var $locationID;
	var $locationName;
	var $so;
	var $active;
	...
	function __construct() {
		...
		$this->TableVar = 'mtr_locations';
		$this->TableName = 'mtr_locations';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`mtr_locations`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// locationID
		$this->locationID = new cField('mtr_locations', 'mtr_locations', 'x_locationID', 'locationID', '`locationID`', '`locationID`', 3, -1, FALSE, '`locationID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// locationName
		$this->locationName = new cField('mtr_locations', 'mtr_locations', 'x_locationName', 'locationName', '`locationName`', '`locationName`', 200, -1, FALSE, '`locationName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->locationName->Sortable = TRUE; // Allow sort
		$this->fields['locationName'] = &$this->locationName;

		// so
		$this->so = new cField('mtr_locations', 'mtr_locations', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('mtr_locations', 'mtr_locations', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`mtr_locations`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "mtr_locationsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "mtr_locationsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("mtr_locationsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("mtr_locationsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "mtr_locationsadd.php?" . $this->UrlParm($parm);
		else
			$url = "mtr_locationsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("mtr_locationsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("mtr_locationsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("mtr_locationsdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["locationID"]))
				$arKeys[] = $_POST["locationID"];
			elseif (isset($_GET["locationID"]))
				$arKeys[] = $_GET["locationID"];
			else
