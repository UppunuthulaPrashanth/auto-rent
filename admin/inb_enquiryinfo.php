...
//
// Table class for inb_enquiry
//
class cinb_enquiry extends cTable {
	var $id;
	var $fullname;
	var $_email;
	var $service;
	...
	function __construct() {
		...
		$this->TableVar = 'inb_enquiry';
		$this->TableName = 'inb_enquiry';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`inb_enquiry`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('inb_enquiry', 'inb_enquiry', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// fullname
		$this->fullname = new cField('inb_enquiry', 'inb_enquiry', 'x_fullname', 'fullname', '`fullname`', '`fullname`', 200, -1, FALSE, '`fullname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fullname->Sortable = TRUE; // Allow sort
		$this->fields['fullname'] = &$this->fullname;

		// email
		$this->_email = new cField('inb_enquiry', 'inb_enquiry', 'x__email', 'email', '`email`', '`email`', 200, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// service
		$this->service = new cField('inb_enquiry', 'inb_enquiry', 'x_service', 'service', '`service`', '`service`', 200, -1, FALSE, '`service`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->service->Sortable = TRUE; // Allow sort
		$this->fields['service'] = &$this->service;

		// message
		$this->message = new cField('inb_enquiry', 'inb_enquiry', 'x_message', 'message', '`message`', '`message`', 201, -1, FALSE, '`message`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->message->Sortable = TRUE; // Allow sort
		$this->fields['message'] = &$this->message;

		// dateCreated
		$this->dateCreated = new cField('inb_enquiry', 'inb_enquiry', 'x_dateCreated', 'dateCreated', '`dateCreated`', ew_CastDateFieldForLike('`dateCreated`', 7, "DB"), 135, 7, FALSE, '`dateCreated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`inb_enquiry`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "inb_enquiryedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "inb_enquiryadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("inb_enquiryview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("inb_enquiryview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "inb_enquiryadd.php?" . $this->UrlParm($parm);
		else
			$url = "inb_enquiryadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("inb_enquiryedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("inb_enquiryadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("inb_enquirydelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->service->Exportable) $Doc->ExportCaption($this->service);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->service->Exportable) $Doc->ExportField($this->service);
