...
$pages_static_view = NULL; // Initialize page object first

class cpages_static_view extends cpages_static {
	...
	// Page object name
	var $PageObjName = 'pages_static_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["_pageID"] <> "") {
			$this->RecKey["_pageID"] = $_GET["_pageID"];
			$KeyUrl .= "&amp;_pageID=" . urlencode($this->RecKey["_pageID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["_pageID"] <> "") {
				...
				$this->RecKey["_pageID"] = $this->_pageID->QueryStringValue;
			} elseif (@$_POST["_pageID"] <> "") {
				$this->_pageID->setFormValue($_POST["_pageID"]);
				$this->RecKey["_pageID"] = $this->_pageID->FormValue;
			} else {
				$sReturnUrl = "pages_staticlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "pages_staticlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "pages_staticlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($pages_static_view)) $pages_static_view = new cpages_static_view();

// Page init
$pages_static_view->Page_Init();

// Page main
$pages_static_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pages_static_view->Page_Render();
?>
...
var CurrentForm = fpages_staticview = new ew_Form("fpages_staticview", "view");

// Form_CustomValidate event
fpages_staticview.Form_CustomValidate = 
...
fpages_staticview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php if ($pages_static->Export == "") { ?>
<div class="ewToolbar">
<?php $pages_static_view->ExportOptions->Render("body") ?>
<?php
	foreach ($pages_static_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $pages_static_view->ShowPageHeader(); ?>
<?php
$pages_static_view->ShowMessage();
?>
<form name="fpages_staticview" id="fpages_staticview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pages_static_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pages_static_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="pages_static">
<input type="hidden" name="modal" value="<?php echo intval($pages_static_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($pages_static->_pageID->Visible) { // pageID ?>
	<tr id="r__pageID">
		<td class="col-sm-2"><span id="elh_pages_static__pageID"><?php echo $pages_static->_pageID->FldCaption() ?></span></td>
...
<?php echo $pages_static->_pageID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pages_static->pageTitle->Visible) { // pageTitle ?>
	<tr id="r_pageTitle">
		<td class="col-sm-2"><span id="elh_pages_static_pageTitle"><?php echo $pages_static->pageTitle->FldCaption() ?></span></td>
...
<?php echo $pages_static->pageTitle->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pages_static->subTitle->Visible) { // subTitle ?>
	<tr id="r_subTitle">
		<td class="col-sm-2"><span id="elh_pages_static_subTitle"><?php echo $pages_static->subTitle->FldCaption() ?></span></td>
...
<?php echo $pages_static->subTitle->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pages_static->headerBG->Visible) { // headerBG ?>
	<tr id="r_headerBG">
		<td class="col-sm-2"><span id="elh_pages_static_headerBG"><?php echo $pages_static->headerBG->FldCaption() ?></span></td>
		<td data-name="headerBG"<?php echo $pages_static->headerBG->CellAttributes() ?>>
<span id="el_pages_static_headerBG">
<span>
<?php echo ew_GetFileViewTag($pages_static->headerBG, $pages_static->headerBG->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($pages_static->summary->Visible) { // summary ?>
	<tr id="r_summary">
		<td class="col-sm-2"><span id="elh_pages_static_summary"><?php echo $pages_static->summary->FldCaption() ?></span></td>
		<td data-name="summary"<?php echo $pages_static->summary->CellAttributes() ?>>
<span id="el_pages_static_summary">
<span<?php echo $pages_static->summary->ViewAttributes() ?>>
<?php echo $pages_static->summary->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pages_static->image->Visible) { // image ?>
	<tr id="r_image">
		<td class="col-sm-2"><span id="elh_pages_static_image"><?php echo $pages_static->image->FldCaption() ?></span></td>
...
<?php echo $pages_static->image->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($pages_static->Export == "") { ?>
<script type="text/javascript">
fpages_staticview.Init();
</script>
<?php } ?>
<?php
$pages_static_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($pages_static->Export == "") { ?>
...
<?php
$pages_static_view->Page_Terminate();
?>
