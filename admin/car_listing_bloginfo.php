...
$car_listing_blog = NULL;

//
// Table class for car_listing_blog
//
class ccar_listing_blog extends cTable {
	var $id;
	var $image;
	var $linkTo;
	var $so;
	...
	function __construct() {
		...
		$this->TableVar = 'car_listing_blog';
		$this->TableName = 'car_listing_blog';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`car_listing_blog`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('car_listing_blog', 'car_listing_blog', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// image
		$this->image = new cField('car_listing_blog', 'car_listing_blog', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// linkTo
		$this->linkTo = new cField('car_listing_blog', 'car_listing_blog', 'x_linkTo', 'linkTo', '`linkTo`', '`linkTo`', 200, -1, FALSE, '`linkTo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->linkTo->Sortable = TRUE; // Allow sort
		$this->fields['linkTo'] = &$this->linkTo;

		// so
		$this->so = new cField('car_listing_blog', 'car_listing_blog', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('car_listing_blog', 'car_listing_blog', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`car_listing_blog`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "car_listing_bloglist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "car_listing_blogedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "car_listing_blogadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("car_listing_blogview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("car_listing_blogview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "car_listing_blogadd.php?" . $this->UrlParm($parm);
		else
			$url = "car_listing_blogadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("car_listing_blogedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("car_listing_blogadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("car_listing_blogdelete.php", $this->UrlParm());
	}
