...
//
// Table class for smtp_details
//
class csmtp_details extends cTable {
	var $id;
	var $host;
	var $username;
	var $password;
	...
	function __construct() {
		...
		$this->TableVar = 'smtp_details';
		$this->TableName = 'smtp_details';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`smtp_details`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('smtp_details', 'smtp_details', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// host
		$this->host = new cField('smtp_details', 'smtp_details', 'x_host', 'host', '`host`', '`host`', 200, -1, FALSE, '`host`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->host->Sortable = TRUE; // Allow sort
		$this->fields['host'] = &$this->host;

		// username
		$this->username = new cField('smtp_details', 'smtp_details', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->username->Sortable = TRUE; // Allow sort
		$this->fields['username'] = &$this->username;

		// password
		$this->password = new cField('smtp_details', 'smtp_details', 'x_password', 'password', '`password`', '`password`', 200, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// port
		$this->port = new cField('smtp_details', 'smtp_details', 'x_port', 'port', '`port`', '`port`', 200, -1, FALSE, '`port`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->port->Sortable = TRUE; // Allow sort
		$this->fields['port'] = &$this->port;

		// secured_type
		$this->secured_type = new cField('smtp_details', 'smtp_details', 'x_secured_type', 'secured_type', '`secured_type`', '`secured_type`', 200, -1, FALSE, '`secured_type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->secured_type->Sortable = TRUE; // Allow sort
		$this->fields['secured_type'] = &$this->secured_type;

		// active
		$this->active = new cField('smtp_details', 'smtp_details', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`smtp_details`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "smtp_detailsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "smtp_detailsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("smtp_detailsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("smtp_detailsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "smtp_detailsadd.php?" . $this->UrlParm($parm);
		else
			$url = "smtp_detailsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("smtp_detailsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("smtp_detailsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("smtp_detailsdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->secured_type->setDbValue($rs->fields('secured_type'));
	...
	function RenderEditRow() {
		...
		$this->host->EditValue = $this->host->CurrentValue;
		...
		$this->secured_type->EditValue = $this->secured_type->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->secured_type->Exportable) $Doc->ExportCaption($this->secured_type);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->secured_type->Exportable) $Doc->ExportField($this->secured_type);
