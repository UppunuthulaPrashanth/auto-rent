...
//
// Table class for promotions
//
class cpromotions extends cTable {
	var $promotionID;
	var $title;
	var $subTitle;
	var $image;
	...
	function __construct() {
		...
		$this->TableVar = 'promotions';
		$this->TableName = 'promotions';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`promotions`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// promotionID
		$this->promotionID = new cField('promotions', 'promotions', 'x_promotionID', 'promotionID', '`promotionID`', '`promotionID`', 3, -1, FALSE, '`promotionID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->promotionID->Sortable = TRUE; // Allow sort
		$this->promotionID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['promotionID'] = &$this->promotionID;

		// title
		$this->title = new cField('promotions', 'promotions', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// subTitle
		$this->subTitle = new cField('promotions', 'promotions', 'x_subTitle', 'subTitle', '`subTitle`', '`subTitle`', 200, -1, FALSE, '`subTitle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->subTitle->Sortable = TRUE; // Allow sort
		$this->fields['subTitle'] = &$this->subTitle;

		// image
		$this->image = new cField('promotions', 'promotions', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// homeImage
		$this->homeImage = new cField('promotions', 'promotions', 'x_homeImage', 'homeImage', '`homeImage`', '`homeImage`', 200, -1, TRUE, '`homeImage`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->homeImage->Sortable = TRUE; // Allow sort
		$this->fields['homeImage'] = &$this->homeImage;

		// summary
		$this->summary = new cField('promotions', 'promotions', 'x_summary', 'summary', '`summary`', '`summary`', 201, -1, FALSE, '`summary`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->summary->Sortable = TRUE; // Allow sort
		$this->fields['summary'] = &$this->summary;

		// createdDate
		$this->createdDate = new cField('promotions', 'promotions', 'x_createdDate', 'createdDate', '`createdDate`', ew_CastDateFieldForLike('`createdDate`', 7, "DB"), 135, 7, FALSE, '`createdDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// so
		$this->so = new cField('promotions', 'promotions', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('promotions', 'promotions', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`promotions`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->promotionID->setDbValue($conn->Insert_ID());
			$rs['promotionID'] = $this->promotionID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('promotionID', $rs))
				ew_AddFilter($where, ew_QuotedName('promotionID', $this->DBID) . '=' . ew_QuotedValue($rs['promotionID'], $this->promotionID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->promotionID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->promotionID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@promotionID@", ew_AdjustSql($this->promotionID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "promotionsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "promotionsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("promotionsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("promotionsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "promotionsadd.php?" . $this->UrlParm($parm);
		else
			$url = "promotionsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("promotionsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("promotionsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("promotionsdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "promotionID:" . ew_VarToJson($this->promotionID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->promotionID->CurrentValue)) {
			$sUrl .= "promotionID=" . urlencode($this->promotionID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["promotionID"]))
				$arKeys[] = $_POST["promotionID"];
			elseif (isset($_GET["promotionID"]))
				$arKeys[] = $_GET["promotionID"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->promotionID->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->promotionID->setDbValue($rs->fields('promotionID'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->promotionID->Exportable) $Doc->ExportCaption($this->promotionID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->promotionID->Exportable) $Doc->ExportField($this->promotionID);
