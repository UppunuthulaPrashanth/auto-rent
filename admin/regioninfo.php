...
class cregion extends cTable {
	...
	function __construct() {
		...
		// regionID
		$this->regionID = new cField('region', 'region', 'x_regionID', 'regionID', '`regionID`', '`regionID`', 3, -1, FALSE, '`regionID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// locationID
		$this->locationID = new cField('region', 'region', 'x_locationID', 'locationID', '`locationID`', '`locationID`', 3, -1, FALSE, '`locationID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// regionName
		$this->regionName = new cField('region', 'region', 'x_regionName', 'regionName', '`regionName`', '`regionName`', 200, -1, FALSE, '`regionName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->regionName->Sortable = TRUE; // Allow sort
		$this->fields['regionName'] = &$this->regionName;

		// so
		$this->so = new cField('region', 'region', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('region', 'region', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`region`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->regionID->setDbValue($conn->Insert_ID());
			$rs['regionID'] = $this->regionID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('regionID', $rs))
				ew_AddFilter($where, ew_QuotedName('regionID', $this->DBID) . '=' . ew_QuotedValue($rs['regionID'], $this->regionID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->regionID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->regionID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@regionID@", ew_AdjustSql($this->regionID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "regionedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "regionadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("regionview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("regionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "regionadd.php?" . $this->UrlParm($parm);
		else
			$url = "regionadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("regionedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("regionadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "regionID:" . ew_VarToJson($this->regionID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->regionID->CurrentValue)) {
			$sUrl .= "regionID=" . urlencode($this->regionID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["regionID"]))
				$arKeys[] = $_POST["regionID"];
			elseif (isset($_GET["regionID"]))
				$arKeys[] = $_GET["regionID"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->regionID->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->regionName->setDbValue($rs->fields('regionName'));
	...
	function RenderEditRow() {
		...
		$this->regionName->EditValue = $this->regionName->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->regionName->Exportable) $Doc->ExportCaption($this->regionName);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->regionName->Exportable) $Doc->ExportField($this->regionName);
