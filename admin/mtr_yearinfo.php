...
//
// Table class for mtr_year
//
class cmtr_year extends cTable {
	var $yearID;
	var $year;
	var $so;
	var $active;
	...
	function __construct() {
		...
		$this->TableVar = 'mtr_year';
		$this->TableName = 'mtr_year';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`mtr_year`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// yearID
		$this->yearID = new cField('mtr_year', 'mtr_year', 'x_yearID', 'yearID', '`yearID`', '`yearID`', 3, -1, FALSE, '`yearID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// year
		$this->year = new cField('mtr_year', 'mtr_year', 'x_year', 'year', '`year`', '`year`', 200, -1, FALSE, '`year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->year->Sortable = TRUE; // Allow sort
		$this->fields['year'] = &$this->year;

		// so
		$this->so = new cField('mtr_year', 'mtr_year', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('mtr_year', 'mtr_year', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`mtr_year`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->yearID->setDbValue($conn->Insert_ID());
			$rs['yearID'] = $this->yearID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('yearID', $rs))
				ew_AddFilter($where, ew_QuotedName('yearID', $this->DBID) . '=' . ew_QuotedValue($rs['yearID'], $this->yearID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->yearID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->yearID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@yearID@", ew_AdjustSql($this->yearID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "mtr_yearedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "mtr_yearadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("mtr_yearview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("mtr_yearview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "mtr_yearadd.php?" . $this->UrlParm($parm);
		else
			$url = "mtr_yearadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("mtr_yearedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("mtr_yearadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("mtr_yeardelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "yearID:" . ew_VarToJson($this->yearID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->yearID->CurrentValue)) {
			$sUrl .= "yearID=" . urlencode($this->yearID->CurrentValue);
		} else {
