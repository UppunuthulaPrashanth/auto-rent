...
class cfaqs extends cTable {
	...
	function __construct() {
		...
		// faqID
		$this->faqID = new cField('faqs', 'faqs', 'x_faqID', 'faqID', '`faqID`', '`faqID`', 3, -1, FALSE, '`faqID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->faqID->Sortable = TRUE; // Allow sort
		$this->faqID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['faqID'] = &$this->faqID;

		// question
		$this->question = new cField('faqs', 'faqs', 'x_question', 'question', '`question`', '`question`', 200, -1, FALSE, '`question`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->question->Sortable = TRUE; // Allow sort
		$this->fields['question'] = &$this->question;

		// answer
		$this->answer = new cField('faqs', 'faqs', 'x_answer', 'answer', '`answer`', '`answer`', 201, -1, FALSE, '`answer`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->answer->Sortable = TRUE; // Allow sort
		$this->fields['answer'] = &$this->answer;

		// so
		$this->so = new cField('faqs', 'faqs', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('faqs', 'faqs', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`faqs`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->faqID->setDbValue($conn->Insert_ID());
			$rs['faqID'] = $this->faqID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('faqID', $rs))
				ew_AddFilter($where, ew_QuotedName('faqID', $this->DBID) . '=' . ew_QuotedValue($rs['faqID'], $this->faqID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->faqID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->faqID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@faqID@", ew_AdjustSql($this->faqID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "faqsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "faqsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("faqsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("faqsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "faqsadd.php?" . $this->UrlParm($parm);
		else
			$url = "faqsadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("faqsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("faqsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "faqID:" . ew_VarToJson($this->faqID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->faqID->CurrentValue)) {
			$sUrl .= "faqID=" . urlencode($this->faqID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["faqID"]))
				$arKeys[] = $_POST["faqID"];
			elseif (isset($_GET["faqID"]))
				$arKeys[] = $_GET["faqID"];
			else
	...
	function LoadListRowValues(&$rs) {
		$this->faqID->setDbValue($rs->fields('faqID'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->faqID->Exportable) $Doc->ExportCaption($this->faqID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->faqID->Exportable) $Doc->ExportField($this->faqID);
