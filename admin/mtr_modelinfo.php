...
//
// Table class for mtr_model
//
class cmtr_model extends cTable {
	var $modelID;
	var $makeID;
	var $bodyTypeID;
	var $model;
	...
	function __construct() {
		...
		$this->TableVar = 'mtr_model';
		$this->TableName = 'mtr_model';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`mtr_model`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// modelID
		$this->modelID = new cField('mtr_model', 'mtr_model', 'x_modelID', 'modelID', '`modelID`', '`modelID`', 3, -1, FALSE, '`modelID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// makeID
		$this->makeID = new cField('mtr_model', 'mtr_model', 'x_makeID', 'makeID', '`makeID`', '`makeID`', 3, -1, FALSE, '`makeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// bodyTypeID
		$this->bodyTypeID = new cField('mtr_model', 'mtr_model', 'x_bodyTypeID', 'bodyTypeID', '`bodyTypeID`', '`bodyTypeID`', 3, -1, FALSE, '`bodyTypeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// model
		$this->model = new cField('mtr_model', 'mtr_model', 'x_model', 'model', '`model`', '`model`', 200, -1, FALSE, '`model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->model->Sortable = TRUE; // Allow sort
		$this->fields['model'] = &$this->model;

		// so
		$this->so = new cField('mtr_model', 'mtr_model', 'x_so', 'so', '`so`', '`so`', 16, -1, FALSE, '`so`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->so->Sortable = TRUE; // Allow sort
		$this->so->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['so'] = &$this->so;

		// active
		$this->active = new cField('mtr_model', 'mtr_model', 'x_active', 'active', '`active`', '`active`', 16, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`mtr_model`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->modelID->setDbValue($conn->Insert_ID());
			$rs['modelID'] = $this->modelID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('modelID', $rs))
				ew_AddFilter($where, ew_QuotedName('modelID', $this->DBID) . '=' . ew_QuotedValue($rs['modelID'], $this->modelID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->modelID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->modelID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@modelID@", ew_AdjustSql($this->modelID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "mtr_modeledit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "mtr_modeladd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("mtr_modelview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("mtr_modelview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "mtr_modeladd.php?" . $this->UrlParm($parm);
		else
			$url = "mtr_modeladd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("mtr_modeledit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("mtr_modeladd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("mtr_modeldelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "modelID:" . ew_VarToJson($this->modelID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->modelID->CurrentValue)) {
			$sUrl .= "modelID=" . urlencode($this->modelID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["modelID"]))
				$arKeys[] = $_POST["modelID"];
			elseif (isset($_GET["modelID"]))
				$arKeys[] = $_GET["modelID"];
			else
